...
import { EXAMPLES as DEFAULT_EXAMPLES, AUTOCOMPLETE_SEED, CATEGORIES } from '@/config/search';
...
interface SearchBarProps {
  ...
  placeholder?: string;
  autoFocus?: boolean;
  /** Override the animated placeholder examples. Defaults to AbleLayer EXAMPLES. */
  examples?: string[];
  /** Override the autocomplete seed/categories/endpoint. Defaults to AbleLayer config. */
...
export function SearchBar({
  ...
  autocompleteConfig = DEFAULT_AUTOCOMPLETE_CONFIG,
  ...
  const { suggestions, activeIndex, setActiveIndex, dismiss, resetDismissed } =
    useAutocomplete(value, autocompleteConfig);

  // Animated placeholder — only when no explicit placeholder prop is given
  const animated = placeholder === undefined;
    ...
    <form onSubmit={handleSubmit} className="w-full">
      <div className="relative w-full">
        ...
        <Input
          ...
          placeholder={animated ? displayPlaceholder : placeholder}
          className={cn(
            ...
            {suggestions.map((s, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span>
                    <span className="font-normal text-foreground/70">{typed}</span>
