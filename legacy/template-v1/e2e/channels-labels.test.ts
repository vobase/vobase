...
describe('Labels CRUD', () => {
  ...
  test('update label', async () => {
    const { status, json } = await api('PATCH', `/api/messaging/labels/${testLabelId}`, {
  ...
  test('delete label', async () => {
    const { status, json } = await api('DELETE', `/api/messaging/labels/${testLabelId}`)
    expect(status).toBe(200)
...
describe('Conversation labels', () => {
  ...
  test('get labels for conversation', async () => {
    const { status, json } = await api('GET', `/api/messaging/conversations/${convId}/labels`)
    expect(status).toBe(200)
  ...
  test('add label to conversation creates activity event', async () => {
    // Add label
    const { status } = await api('POST', `/api/messaging/conversations/${convId}/labels`, { labelIds: ['lbl-bug'] })
    expect(status).toBe(200)

    // Verify label assigned
    const { json: labels } = await api('GET', `/api/messaging/conversations/${convId}/labels`)
    const assigned = labels as Array<{ id: string }>
    expect(assigned.some((l) => l.id === 'lbl-bug')).toBe(true)

    // Verify activity event created
    const { json: msgs } = await api('GET', `/api/messaging/conversations/${convId}/messages?limit=5`)
    const messages = (msgs as { messages: Array<{ content: string }> }).messages
    expect(messages.some((m) => m.content === 'label.added')).toBe(true)
  })

  test('remove label from conversation creates activity event', async () => {
    const { status } = await api('DELETE', `/api/messaging/conversations/${convId}/labels/lbl-bug`)
    expect(status).toBe(200)
  ...
  test('adding duplicate label is idempotent', async () => {
    ...
    const { status } = await api('POST', `/api/messaging/conversations/${convId}/labels`, { labelIds: ['lbl-vip'] })
    expect(status).toBe(200) // onConflictDoNothing

    // Only one VIP label
    const { json: labels } = await api('GET', `/api/messaging/conversations/${convId}/labels`)
    const vipCount = (labels as Array<{ id: string }>).filter((l) => l.id === 'lbl-vip').length
...
describe('Conversation list includes labels', () => {
  test('attention queue returns labels array', async () => {
    const { status, json } = await api('GET', '/api/messaging/conversations/attention?limit=10')
    expect(status).toBe(200)
  ...
  test('ai-active list returns labels array', async () => {
    const { json } = await api('GET', '/api/messaging/conversations/ai-active?limit=10')
  ...
  test('resolved list returns labels array', async () => {
    const { json } = await api('GET', '/api/messaging/conversations/resolved?limit=10')
    const convs = json as Array<{ labels: unknown[] }>
    for (const c of convs) {
      expect(Array.isArray(c.labels)).toBe(true)
    }
...
describe('Staff reply and private notes', () => {
  ...
  test('send staff reply persists message', async () => {
    const { status, json } = await api('POST', `/api/messaging/conversations/${convId}/reply`, {
    ...
    // Verify in messages
    const { json: msgs } = await api('GET', `/api/messaging/conversations/${convId}/messages?limit=10`)
    const messages = (msgs as { messages: Array<{ content: string; private: boolean }> }).messages
    expect(messages.some((m) => m.content === 'E2E test reply message')).toBe(true)
  })

  test('send private note does not leak to lastMessageContent', async () => {
    const { status } = await api('POST', `/api/messaging/conversations/${convId}/reply`, {
    ...
    const { json: convs } = await api('GET', '/api/messaging/conversations/attention?limit=50')
    const conv = (convs as Array<{ id: string; lastMessageContent: string }>).find((c) => c.id === convId)
...
describe('Message timeline activity filtering', () => {
  test('activity messages include label events', async () => {
    const convId = 'sess-for-completion'
    const { json: msgs } = await api('GET', `/api/messaging/conversations/${convId}/messages?limit=50`)
    const messages = (
      msgs as {
    ...
    const labelEvents = activities.filter((m) => m.content === 'label.added' || m.content === 'label.removed')
  ...
  test('non-visible activity events exist but are filtered client-side', async () => {
    ...
    const convId = 'sess-human-low'
    const { json: msgs } = await api('GET', `/api/messaging/conversations/${convId}/messages?limit=100`)
    const messages = (
      msgs as {
        messages: Array<{ messageType: string; content: string }>
      }
    ...
    const nonVisible = activities.filter((m) => m.content === 'message.read' || m.content === 'message.outbound_queued')
...
describe('Real AI agent chat (web streaming)', () => {
  test('start a new chat conversation', async () => {
    const { status, json } = await api('POST', '/api/agents/chat/ep-web-booking/start')
    expect(status).toBe(200)
    ...
    const res = await fetch(`${BASE}/api/agents/chat/ep-web-booking/stream`, {
      ...
      body: JSON.stringify({
        messages: [
          {
            role: 'user',
            content: 'Hello, I need to book a dental checkup appointment please.',
          },
        ],
      }),
    })
    ...
    const res = await fetch(`${BASE}/api/agents/chat/ep-web-booking/stream`, {
      ...
      body: JSON.stringify({
        messages: [
          ...
          {
            role: 'assistant',
            content: "I'd be happy to help you book a dental checkup! What date works best for you?",
          },
  ...
  test('human/held mode blocks AI streaming', async () => {
    ...
    const { json: conv } = await api('GET', '/api/messaging/conversations/sess-held-mode')
...
describe('Auth required on new endpoints', () => {
  const endpoints = ['/api/messaging/labels', '/api/messaging/conversations/sess-human-low/labels']
