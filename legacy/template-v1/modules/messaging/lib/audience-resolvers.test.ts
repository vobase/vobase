...
import { automationRecipients, automationRuleSteps, automationRules, channelInstances, contacts } from '../schema'
import {
...
describe('audience-resolvers registry', () => {
  ...
  it('throws when registering the same name twice', () => {
    ...
    expect(() => registerAudienceResolver('dup', async () => [])).toThrow(/already registered/)
  })
...
describe('audience-resolvers engine integration', () => {
  ...
  async function seedBaseRule(overrides: Partial<typeof automationRules.$inferInsert> = {}) {
  ...
  it('resolver values take precedence over mapped variables on key collision', async () => {
    ...
    registerAudienceResolver('vip-override', async () => [{ contactId: 'c-alice', variables: { '1': 'VIP Alice' } }])
  ...
  it('throws a clear error when audienceResolverName references an unregistered resolver', async () => {
    ...
    await expect(evaluateRecurringRules(new Date('2026-04-17T02:00:00Z'))).rejects.toThrow(
      /Unknown audience resolver: does-not-exist/,
    )
  })
