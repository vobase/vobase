...
describe('getCaptionForContentType', () => {
  it('returns placeholder for audio without storage', async () => {
    const result = await getCaptionForContentType('audio', undefined, undefined, undefined)
    expect(result).toBe('(voice message — transcription not yet available)')
  })

  it('returns placeholder for video without storage', async () => {
    const result = await getCaptionForContentType('video', undefined, undefined, undefined)
    expect(result).toBe('(video — description not yet available)')
  })

  it('returns null for unknown content type', async () => {
    const result = await getCaptionForContentType('sticker', undefined, undefined, undefined)
    expect(result).toBeNull()
  })

  it('returns null for image without storage', async () => {
    const result = await getCaptionForContentType('image', 'some/key.jpg', 'image/jpeg', undefined)
    expect(result).toBeNull()
  })
  ...
  it('returns null for image without storageKey', async () => {
    ...
    const result = await getCaptionForContentType('image', undefined, 'image/jpeg', mockStorage)
    expect(result).toBeNull()
  })

  it('returns null for document without storage', async () => {
    const result = await getCaptionForContentType('document', 'some/key.pdf', 'application/pdf', undefined)
    expect(result).toBeNull()
  })
})
...
describe('captionDocument — text file fast path', () => {
  it('reads small text files directly without AI', async () => {
    ...
    const result = await captionDocument('conv/msg/data.csv', 'text/csv', mockStorage)
  ...
  it('returns null for empty text files', async () => {
    ...
    const { captionDocument } = await import('./caption')
    const result = await captionDocument('conv/msg/empty.txt', 'text/plain', mockStorage)
  ...
  it('returns null on download failure', async () => {
    ...
    const { captionDocument } = await import('./caption')
    const result = await captionDocument('conv/msg/missing.txt', 'text/plain', mockStorage)
