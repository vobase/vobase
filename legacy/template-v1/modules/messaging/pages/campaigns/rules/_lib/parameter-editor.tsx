...
function AudienceFilterPreview({ value }: { value: unknown }) {
  if (!isStoredAudienceFilter(value)) {
    return <p className="text-muted-foreground rounded-md border bg-muted/40 px-3 py-2 text-sm">—</p>
  }
  ...
  return (
    <div className="flex flex-col gap-1 rounded-md border bg-muted/40 px-3 py-2 text-sm">
      {parts.length > 0 && <p className="text-muted-foreground">{parts.join(' · ')}</p>}
      {attributes.length > 0 ? (
        <ul className="flex flex-col gap-0.5 font-mono text-xs">
          {attributes.map((attr, i) => (
            <li key={`${attr.key}-${i.toString()}`}>
              <span>{attr.key}</span>{' '}
              <span className="text-muted-foreground">{OPERATOR_LABELS[attr.op ?? 'eq'] ?? attr.op ?? '='}</span>{' '}
...
export function ParameterEditor({ schema, values, onSave }: ParameterEditorProps) {
      ...
      {keys.map((key) => {
        ...
        if (entry.type === 'boolean') {
          return (
            <div key={key} className="flex items-center justify-between gap-4">
              ...
              <Switch id={`param-${key}`} checked={Boolean(val)} onCheckedChange={(v) => set(key, v)} />
            </div>
          )
        }
        ...
        if (entry.type === 'select' && entry.options) {
          return (
            <div key={key} className="flex flex-col gap-1.5">
              ...
              <Select value={String(val ?? '')} onValueChange={(v) => set(key, v)}>
        ...
        return (
          <div key={key} className="flex flex-col gap-1.5">
            ...
            <Input id={`param-${key}`} value={String(val ?? '')} onChange={(e) => set(key, e.target.value)} />
          </div>
        )
      })}
