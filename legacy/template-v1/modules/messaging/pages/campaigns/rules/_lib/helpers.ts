...
export function cronToHuman(cron: string | null | undefined): string {
  ...
  if (everyNMinutes && hourStr === '*' && domStr === '*' && monStr === '*' && dowStr === '*') {
    return `Every ${everyNMinutes[1]} minutes`
  }

  if (minStr === '0' && hourStr === '*' && domStr === '*' && monStr === '*' && dowStr === '*') {
    return 'Every hour'
  }
  ...
  const time = formatTimeOfDay(`${hour.toString().padStart(2, '0')}:${minute.toString().padStart(2, '0')}`)
  ...
  if (domStr === '*' && monStr === '*' && (dowStr === '0,6' || dowStr === '6,0')) {
