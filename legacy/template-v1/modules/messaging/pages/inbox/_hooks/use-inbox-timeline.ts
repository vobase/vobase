...
async function fetchTimelinePage(contactId: string, before?: string): Promise<TimelinePage> {
  ...
  if (!res.ok) return { messages: [], hasMore: false, conversations: [], channels: [] }
...
async function fetchTeamMembers(): Promise<Array<{ id: string; name: string }>> {
...
export function useInboxTimeline(
  ...
  const { data: timelineData, hasNextPage } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }: { pageParam: string | undefined }) => fetchTimelinePage(contactId, pageParam),
  ...
  const allMessages = useMemo(() => timelineData?.pages.flatMap((p) => p.messages) ?? [], [timelineData])

  const allConversations = useMemo(() => timelineData?.pages[0]?.conversations ?? [], [timelineData])

  const channels = useMemo(() => timelineData?.pages[0]?.channels ?? [], [timelineData])
  ...
  const selectedTabChannelId = useMemo(() => {
    ...
    if (channelTabOverride && channels.some((c) => c.id === channelTabOverride)) return channelTabOverride
    ...
    const sorted = [...channels].sort((a, b) => (activity.get(b.id) ?? 0) - (activity.get(a.id) ?? 0))
    return sorted[0]?.id ?? null
  }, [channelTabOverride, channels, allConversations])

  const sortedConversations = useMemo(
    () => [...allConversations].sort((a, b) => new Date(a.startedAt).getTime() - new Date(b.startedAt).getTime()),
    [allConversations],
  )

  const filteredConversations = useMemo(() => {
    if (!selectedTabChannelId) return sortedConversations
    return sortedConversations.filter((i) => i.channelInstanceId === selectedTabChannelId)
  ...
  const activeChannelConversation = useMemo(() => {
    ...
    return filteredConversations.find((i) => i.status === 'active' || i.status === 'resolving') ?? null
  ...
  const selectedChannel = useMemo(
    () => channels.find((c) => c.id === selectedTabChannelId) ?? channels[0] ?? null,
