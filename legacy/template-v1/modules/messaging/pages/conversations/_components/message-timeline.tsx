import { CheckCircle2Icon, Loader2Icon, MessageSquareIcon, XCircleIcon } from 'lucide-react'
...
function ConversationOutcome({ conversation }: { conversation: TimelineConversation }) {
  const icon =
  ...
  const label = conversation.status === 'failed' ? 'Failed' : outcomeLabel(conversation.outcome)
...
export const MessageTimeline = memo(function MessageTimeline({
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      rafId = requestAnimationFrame(() => {
        const sentinels = scrollEl.querySelectorAll<HTMLElement>('[data-conversation-sentinel]')
        ...
        if (atBottom) {
          const lastId = sentinels[sentinels.length - 1].dataset.conversationSentinel ?? null
          if (lastId) {
        ...
        const focalLine = scrollMax > 0 ? containerHeight * 0.33 : containerHeight * 0.85
  ...
  const groups = useMemo(() => {
    ...
    for (const msg of sorted) {
      ...
      const isNewConversation = !currentGroup || msg.conversationId !== currentGroup.conversationId
      ...
      if (isNewConversation) {
        ...
        currentGroup = {
          ...
          items: [],
          isScrollAnchor: !!currentConversationId && msg.conversationId === currentConversationId && isFirst,
          isTerminal: conversation?.status === 'resolved' || conversation?.status === 'failed',
        }
    ...
    <Conversation className="h-full">
      <ConversationContent className="gap-10 px-4 py-4">
        ...
        {groups.map((group) => {
          ...
          const activeLine = group.channelType ? CHANNEL_LINE_ACTIVE[group.channelType] : undefined

          return (
            <div key={`ix-${group.conversationId}`}>
              {/* Scroll sentinel */}
              <div data-conversation-sentinel={group.conversationId} className="h-0" />
              {/* Scroll anchor for initial load */}
              {group.isScrollAnchor && <div ref={currentAnchorRef} className="h-0" />}
                  ...
                  {group.items.map((item) =>
                    item.type === 'date' ? (
                      ...
                      <MessageItem
                        ...
                        scores={scoresMap?.get(item.msg.conversationId)?.get(item.msg.id)}
                      />
                    ),
                  )}
                </div>
...
const MessageItem = memo(function MessageItem({
  ...
  if (isMe) {
    ...
    return (
      <OutgoingMessage message={message} sender={sender} onRetry={onRetry} align="right" channelType={channelType} />
    )
  }

  if (message.messageType === 'incoming') {
    return <IncomingMessage message={message} sender={sender} channelType={channelType} />
  }

  // Agent, other staff, system → left-aligned
  return (
    <OutgoingMessage message={message} sender={sender} onRetry={onRetry} channelType={channelType} scores={scores} />
  )
})
