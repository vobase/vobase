...
import { BotIcon, CornerUpLeftIcon, LockIcon, ShieldCheckIcon, UserIcon } from 'lucide-react'
import { memo } from 'react'

import { CardRenderer } from '@/components/ai-elements/card-renderer'
import { Message, MessageContent, MessageResponse } from '@/components/ai-elements/message'
import { MessageQualityIndicator, type MessageScoreGroup } from '@/components/chat/message-quality'
...
import { isWhatsAppContent, WhatsAppEchoAttribution, WhatsAppOutgoingContent } from './whatsapp-outgoing'
...
export const OutgoingMessage = memo(function OutgoingMessage({
  ...
  const defaultLabel = isAgent ? 'Agent' : isStaff ? 'Staff' : isSystem ? 'System' : 'Unknown'
  ...
  const bubbleClass = cn(
    ...
    !isFailed && isPrivate && 'border border-violet-200 bg-violet-50/80 dark:border-violet-800 dark:bg-violet-950/30',
  ...
  const content = (
    <MessageContent className={bubbleClass}>
      ...
      ) : message.contentType === 'interactive' && (message.contentData as Record<string, unknown>)?.card ? (
        <CardRenderer
          card={(message.contentData as Record<string, unknown>).card as CardElement}
          readOnly
      ...
      {isFailed && message.failureReason && <span className="text-xs text-destructive">{message.failureReason}</span>}
    </MessageContent>
  )
  ...
  const meta = (
    <div className={cn('flex items-center gap-1.5 text-xs text-muted-foreground', align === 'right' && 'justify-end')}>
      <span className={cn('font-medium', isAgent ? 'text-primary' : 'text-foreground')}>{senderLabel}</span>
      {isDraft && (
        <Badge variant="outline" className="h-4 px-1.5 text-[10px] font-medium text-amber-600 border-amber-300">
          Draft
        </Badge>
      )}
    ...
    <Message from="assistant" className={cn('group', className)}>
      <div className="flex items-end gap-2">
        ...
        <div className="flex flex-col gap-1 min-w-0">
          {meta}
          {content}
          {isAgent && scores && scores.scores.length > 0 && <MessageQualityIndicator group={scores} />}
          {reactions}
          {echoAttribution}
        </div>
      </div>
