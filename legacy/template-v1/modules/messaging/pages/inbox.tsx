...
import { BotIcon, CheckCircleIcon, InboxIcon, SearchIcon, XIcon } from 'lucide-react'
...
function messagePreview(row: ContactRow): string | null {
  ...
  if (lastMessageType === 'activity') {
    switch (lastMessageContent) {
      ...
      default:
        return lastMessageContent ? lastMessageContent.replace(/\./g, ' ') : null
    }
  }
...
function ContactRowItem({ row, isFocused }: { row: ContactRow; isFocused?: boolean }) {
      ...
      <div className="flex-1 min-w-0 space-y-0.5">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 min-w-0">
            <span className={cn('truncate text-sm', row.unreadCount > 0 ? 'font-semibold' : 'font-medium')}>
              {displayName}
            </span>
            {row.priority && <PriorityIcon priority={row.priority} />}
            {row.assignee?.startsWith('agent:') && (
              <span className="shrink-0 text-[10px] font-bold text-blue-500">@</span>
            )}
          </div>
          {timeRef ? (
            <RelativeTimeCard date={timeRef} className="text-xs text-muted-foreground whitespace-nowrap shrink-0" />
          ) : (
        ...
        <div className="flex items-center gap-2">
          ...
          {row.unreadCount > 0 && (
            <Badge variant="default" className="h-4 min-w-4 px-1 text-[10px] font-bold shrink-0">
...
function InboxLayout() {
  ...
  const rawRows = tab === 'active' ? (activeData ?? []) : tab === 'on-hold' ? (onHoldData ?? []) : (doneData ?? [])

  const isPending = tab === 'active' ? activePending : tab === 'on-hold' ? onHoldPending : donePending
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      if (panelEl && !panelEl.contains(document.activeElement) && document.activeElement !== document.body) {
        return
      }
  ...
  const emptyProps =
    tab === 'active'
      ? {
          ...
          subtitle: search ? 'No contacts match your search' : 'No active conversations right now',
        }
      : tab === 'on-hold'
        ? {
            icon: <CheckCircleIcon className="h-5 w-5 text-muted-foreground" />,
            title: search ? 'No results' : 'Nothing on hold',
            subtitle: search ? 'No contacts match your search' : 'No contacts are currently on hold',
          }
        : {
            icon: <InboxIcon className="h-5 w-5 text-muted-foreground" />,
            title: search ? 'No results' : 'Nothing resolved yet',
            subtitle: search ? 'No contacts match your search' : 'Resolved contacts will appear here',
          }
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden">
      ...
      <div
        ref={listPanelRef}
        className={cn('w-80 shrink-0 border-r flex flex-col overflow-hidden', hasSelection ? 'hidden lg:flex' : 'flex')}
      >
        {/* Search */}
        ...
        <Tabs value={tab} onValueChange={handleTabChange} className="flex flex-col flex-1 min-h-0">
          ...
          {(['active', 'on-hold', 'done'] as const).map((v) => (
            <TabsContent key={v} value={v} className="min-h-0 flex-1 mt-0 pt-2 overflow-hidden">
                ...
                {isPending ? (
                  ...
                  <div className="divide-y divide-border/50">
                    {filteredRows.map((row, idx) => (
                      <ContactRowItem key={row.id} row={row} isFocused={idx === focusedIndex} />
                    ))}
                  </div>
                )}
              </ScrollArea>
      ...
      {/* Detail pane — flex-1 */}
      <div
        className={cn('flex-1 min-w-0 overflow-hidden', !hasSelection ? 'hidden lg:flex lg:flex-col' : 'flex flex-col')}
      >
        <Outlet />
      </div>
    </div>
