    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[400px]">
        ...
        <DialogFooter>
          ...
          <Button onClick={() => onSave({ title, color, description })} disabled={!title.trim() || isPending}>
            {label ? 'Save' : 'Create'}
          </Button>
        </DialogFooter>
      </DialogContent>
...
function LabelsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: { title: string; color: string; description: string }) => {
      const res = await messagingClient.labels.$post(
        {},
        {
          init: {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: { title: string; color: string; description: string } }) => {
      const res = await messagingClient.labels[':id'].$patch(
        { param: { id } },
        {
          init: {
            body: JSON.stringify(data),
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await messagingClient.labels[':id'].$delete({
        param: { id },
      })
  ...
  function handleSave(data: { title: string; color: string; description: string }) {
    if (editingLabel) {
    ...
    <div className="mx-auto max-w-2xl space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage conversation labels for organization and filtering.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Create labels to organize your conversations.</p>
        ...
        <div className="divide-y rounded-lg border">
          {labelsList.map((label) => (
            <div key={label.id} className="flex items-center gap-3 px-4 py-3 group">
              <div className="h-3 w-3 rounded-full shrink-0" style={{ backgroundColor: label.color ?? '#6b7280' }} />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium">{label.title}</p>
                {label.description && <p className="text-xs text-muted-foreground truncate">{label.description}</p>}
              </div>
...
export const Route = createFileRoute('/_app/messaging/labels')({
