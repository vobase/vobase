...
import { audienceFilterSchema, buildAudienceWhereWithLabels } from '../lib/audience-filter'
import { parseRuleFromPrompt } from '../lib/automation-parse'
import { ParameterSchema, type ParameterSchemaT } from '../lib/parameter-schema'
import { automationExecutions, automationRecipients, automationRuleSteps, automationRules, contacts } from '../schema'
...
const recipientsQuerySchema = z.object({
  ...
  offset: z.coerce.number().int().min(0).default(0),
  status: z.enum(['queued', 'sent', 'delivered', 'read', 'failed', 'skipped', 'replied', 'chaser_paused']).optional(),
})
...
function validateParameterValues(parameters: Record<string, unknown>, schema: ParameterSchemaT) {
...
export const automationHandlers = new Hono()
  ...
  .get('/rules', async (c) => {
    ...
    const [rows, [{ total }]] = await Promise.all([
      db.select().from(automationRules).orderBy(desc(automationRules.createdAt)).limit(limit).offset(offset),
  ...
  .get('/rules/:id/executions', async (c) => {
    ...
    const [rule] = await db.select({ id: automationRules.id }).from(automationRules).where(eq(automationRules.id, id))
    ...
    const conditions = [
      ...
      ...(date_from ? [gte(automationExecutions.firedAt, new Date(date_from))] : []),
      ...(date_to ? [lte(automationExecutions.firedAt, new Date(date_to))] : []),
    ]
  ...
  .get('/executions/:id/recipients', async (c) => {
    ...
    const condition = status
      ? and(eq(automationRecipients.executionId, id), eq(automationRecipients.status, status))
  ...
  .post('/rules/parse', async (c) => {
    ...
    const draft = await parseRuleFromPrompt(parsed.data.prompt, { db }, parsed.data.language ?? 'en')
  ...
  .patch('/rules/:id/steps', async (c) => {
    ...
    const newSteps = await db.transaction(async (tx) => {
      await tx.delete(automationRuleSteps).where(eq(automationRuleSteps.ruleId, id))
  ...
  .post('/rules/:id/pause', async (c) => {
    ...
    await db.update(automationRules).set({ isActive: false, updatedAt: new Date() }).where(eq(automationRules.id, id))
  ...
  .post('/rules/:id/simulate', async (c) => {
    ...
    const filter = filterResult.success ? filterResult.data : audienceFilterSchema.parse({})
