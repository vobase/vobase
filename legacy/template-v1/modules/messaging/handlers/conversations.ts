import { authUser, getCtx, notFound, unauthorized, type VobaseDb } from '@vobase/core'
import { and, asc, desc, eq, inArray, isNull, lt, ne, notInArray, notLike, sql } from 'drizzle-orm'
...
import { channelInstances, contactLabels, contacts, conversations, labels, messageFeedback, messages } from '../schema'

/** Batch-load contact-level labels for a set of contact-keyed rows. */
async function withContactLabels<T extends { id: string }>(
  db: VobaseDb,
  rows: T[],
): Promise<(T & { labels: { id: string; title: string; color: string | null }[] })[]> {
  ...
  const labelMap = new Map<string, { id: string; title: string; color: string | null }[]>()
...
async function getTabContactIds(db: VobaseDb, tab: InboxTab): Promise<string[]> {
  if (tab === 'active') {
    const rows = await db
      .selectDistinct({ contactId: conversations.contactId })
      .from(conversations)
      .where(and(eq(conversations.status, 'active'), eq(conversations.onHold, false)))
  ...
  if (tab === 'on-hold') {
    ...
    const heldRows = await db
      .selectDistinct({ contactId: conversations.contactId })
      .from(conversations)
      .where(and(eq(conversations.status, 'active'), eq(conversations.onHold, true)))
  ...
  const allContactRows = await db.selectDistinct({ contactId: conversations.contactId }).from(conversations)
  return allContactRows.map((r) => r.contactId).filter((id) => !nonTerminalIds.has(id))
}

/** Build contact-level aggregated rows for a tab. */
async function buildContactRows(db: VobaseDb, tab: InboxTab, limit: number, offset: number, userId?: string) {
    ...
    db
      ...
      .where(
        and(
          ...
          tab === 'done' ? inArray(conversations.status, ['resolved', 'failed']) : eq(conversations.status, 'active'),
        ),
      ),
  ...
  const latestMsgMap = new Map<string, { content: string; messageType: string; createdAt: Date }>()
...
export const conversationsDetailHandlers = new Hono()
  ...
  .get('/conversations', async (c) => {
    ...
    if (channelInstanceId) conditions.push(eq(conversations.channelInstanceId, channelInstanceId))
  ...
  .get('/conversations/mine', async (c) => {
    ...
    const rows = await db
      ...
      .where(and(eq(conversations.assignee, user.id), eq(conversations.status, 'active')))
      .orderBy(
  ...
  .get('/conversations/mentions', async (c) => {
    ...
    const mentionRows = await db
      ...
      .from(messages)
      .innerJoin(conversations, eq(messages.conversationId, conversations.id))
      .where(sql`${messages.mentions} @> ${JSON.stringify([{ targetId: user.id, targetType: 'user' }])}::jsonb`)
  ...
  .patch('/conversations/:id', async (c) => {
    ...
    if (body.status === 'resolved' || body.status === 'failed') {
      if (body.status === 'resolved') {
        ...
        await failConversation(db, conversationId, 'Manually failed by user', realtime)
      }
    ...
    if (body.assignee !== undefined && body.assignee !== null) {
      ...
      if (!assignResult.ok) {
        if (assignResult.code === 'CONCURRENCY_CONFLICT') return c.json({ error: assignResult.error }, 409)
    ...
    if (body.onHold !== undefined) {
      ...
      if (!holdResult.ok) {
        if (holdResult.code === 'CONCURRENCY_CONFLICT') return c.json({ error: holdResult.error }, 409)
    ...
    if (body.priority !== undefined) {
      await db.update(conversations).set({ priority: body.priority }).where(eq(conversations.id, conversationId))
    }
  ...
  .post('/conversations/:id/reply', async (c) => {
    ...
    const content = !body.isInternal && channelType !== 'email' ? `[Staff: ${staffName}] ${body.content}` : body.content
  ...
  .post('/conversations/:id/messages/:messageId/feedback', async (c) => {
    ...
    const action = await db.transaction(async (tx) => {
      ...
      if (existing?.rating === body.rating) {
        ...
        await tx.delete(messageFeedback).where(eq(messageFeedback.id, existing.id))
  ...
  .delete('/conversations/:id/messages/:messageId/feedback/:feedbackId', async (c) => {
    const { db, user } = getCtx(c)
    if (!user) throw unauthorized()
    const conversationId = c.req.param('id')
  ...
  .post('/conversations/:id/messages/:mid/retry', async (c) => {
    ...
    const [message] = await db
      .select()
      .from(messages)
      .where(and(eq(messages.id, messageId), eq(messages.conversationId, c.req.param('id'))))
    ...
    await db.update(messages).set({ status: 'queued', failureReason: null }).where(eq(messages.id, messageId))
