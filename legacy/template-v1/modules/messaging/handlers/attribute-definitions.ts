...
const createSchema = z.object({
  key: z
    .string()
    .min(1)
    .regex(/^[a-z][a-z0-9_]*$/, 'Key must be lowercase alphanumeric starting with a letter'),
...
export const attributeDefinitionsHandlers = new Hono()
  ...
  .get('/', async (c) => {
    ...
    const rows = await db.select().from(contactAttributeDefinitions).orderBy(asc(contactAttributeDefinitions.sortOrder))
