...
import { conflict, getCtx, logger, notFound, unauthorized, validation } from '@vobase/core'
...
import { createGoogleDriveConnector, exchangeGoogleCode, getGoogleAuthUrl } from './connectors/google-drive'
import { createSharePointConnector, exchangeSharePointCode, getSharePointAuthUrl } from './connectors/sharepoint'
...
export const knowledgeBaseRoutes = new Hono()
  ...
  .post('/documents', async (c) => {
    ...
    await ctx.storage.bucket(KB_STORAGE_BUCKET).upload(storageKey, new Uint8Array(await file.arrayBuffer()))
  ...
  .patch('/documents/:id/content', async (c) => {
    ...
    const [doc] = await ctx.db.select().from(kbDocuments).where(eq(kbDocuments.id, id))
    if (!doc) throw notFound('Document not found')
    if (doc.status !== 'ready') throw conflict('Document is not ready for editing')
    ...
    if (existingContent && Array.isArray(existingContent)) {
      ...
      oldValue = concatenated ? markdownToPlate(concatenated) : [{ type: 'p', children: [{ text: '' }] }]
    }
    ...
    const allNewChunks = blockChunk(newValue).filter((ch) => ch.content.trim().length > 0)
    ...
    const rechunkedChunks = allNewChunks.filter((ch) => isBlockRangeAffected(ch.blockRange, affectedRanges))
    ...
    const oldChunkRecords = await ctx.db
      ...
      .from(kbChunks)
      .where(eq(kbChunks.documentId, id))
    const oldEmbeddingMap = new Map<string, number[] | null>(oldChunkRecords.map((ch) => [ch.content, ch.embedding]))

    // 7. Embed only changed chunks (outside transaction — API call can't be rolled back)
    const newEmbeddings = rechunkedChunks.length > 0 ? await embedChunks(rechunkedChunks.map((ch) => ch.content)) : []
      ...
      if (allNewChunks.length > 0) {
        await tx.insert(kbChunks).values(
          allNewChunks.map((chunk) => {
            const isChanged = isBlockRangeAffected(chunk.blockRange, affectedRanges)
  ...
  .post('/folders/move', async (c) => {
    ...
    await ctx.db.update(kbDocuments).set({ folder: body.folder }).where(inArray(kbDocuments.id, body.documentIds))
  ...
  .post('/sources/:id/sync', async (c) => {
    ...
    const source = (await ctx.db.select().from(kbSources).where(eq(kbSources.id, sourceId)))[0]
    if (!source) throw notFound('Source not found')
