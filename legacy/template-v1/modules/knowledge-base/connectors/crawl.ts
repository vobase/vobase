 ...
 */
export function createCrawlConnector(config: CrawlConfig, integrations?: IntegrationsService): DocumentSource {
  async function getCredentials(): Promise<{
    ...
    const apiToken = (cfIntegration?.config.apiToken as string) ?? process.env.CLOUDFLARE_API_TOKEN
    const accountId = (cfIntegration?.config.accountId as string) ?? process.env.CLOUDFLARE_ACCOUNT_ID
  ...
  async function startCrawl(): Promise<string> {
    ...
    if (!res.ok) throw new Error(`Crawl start failed: ${res.status} ${await res.raw.text()}`)
    // POST returns { success: true, result: "<job-id>" }
    const data = res.data as { result: string }
    return data.result
  }

  async function pollCrawl(jobId: string): Promise<Array<{ url: string; markdown: string }>> {
    const { apiToken, accountId } = await getCredentials()
    const baseUrl = `https://api.cloudflare.com/client/v4/accounts/${accountId}/browser-rendering`
