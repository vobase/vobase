...
describe('migrateExistingDocuments()', () => {
  ...
  async function insertDoc(id: string, status: string, hasContent = false): Promise<void> {
    await db.insert(kbDocuments).values({
      ...
      mimeType: 'text/plain',
      status,
      content: hasContent ? ([{ type: 'p', children: [{ text: 'existing' }] }] as unknown) : null,
    })
  }

  async function insertChunks(documentId: string, texts: string[]): Promise<void> {
  ...
  it('backfilled content passes plateValueSchema validation', async () => {
    ...
    await insertChunks('schema-1', ['# Heading\n\nSome paragraph text here.', 'Another paragraph with more text.'])
