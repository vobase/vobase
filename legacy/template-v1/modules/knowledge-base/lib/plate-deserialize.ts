...
export function markdownToPlate(md: string): PlateValue {
  const tree = unified().use(remarkParse).use(remarkGfm).parse(md) as unknown as MdRoot
  const blocks = tree.children.flatMap(mdBlockToPlate).filter((b): b is PlateElement => b !== null)
...
function mdInlineToPlate(node: MdNode, marks: InlineMarks = {}): (PlateText | PlateElement)[] {
  switch (node.type) {
    case 'text':
    ...
    case 'emphasis':
      return (node as unknown as MdEmphasis).children.flatMap((n) => mdInlineToPlate(n, { ...marks, italic: true }))
    case 'strong':
      return (node as unknown as MdStrong).children.flatMap((n) => mdInlineToPlate(n, { ...marks, bold: true }))
    case 'delete':
    ...
    case 'link': {
      ...
      return [createLink(link.url, children.length > 0 ? children : [createText(link.url)])]
    }
    case 'image': {
...
function mdBlockToPlate(node: MdNode): PlateElement[] {
  switch (node.type) {
    case 'heading': {
      ...
      return [createHeading(h.depth as 1 | 2 | 3 | 4 | 5 | 6, mdInlinesToPlate(h.children))]
    }
    case 'paragraph':
      return [createParagraph(mdInlinesToPlate((node as unknown as MdParagraph).children))]
    case 'blockquote': {
      const bq = node as unknown as MdBlockquote
      const inlineChildren: (PlateElement | PlateText)[] = bq.children.flatMap((child) => {
        if (child.type === 'paragraph') {
          return mdInlinesToPlate((child as unknown as MdParagraph).children)
        }
        return [createText('')]
      })
      return [createBlockquote(inlineChildren.length > 0 ? inlineChildren : [createText('')])]
    }
    case 'code': {
      const c = node as unknown as MdCode
      return [createCodeBlock((c.value ?? '').split('\n'), c.lang ?? undefined)]
    }
    case 'list':
...
function mdListItemToPlate(node: MdListItem): PlateElement {
  ...
  for (const child of node.children) {
    if (child.type === 'paragraph') {
      licChildren = mdInlinesToPlate((child as unknown as MdParagraph).children)
...
function mdTableToPlate(node: MdTable): PlateElement {
  const rows: PlateElement[] = node.children.map((row, rowIndex) => {
    ...
    const cells = row.children.map((cell) => createTableCell(mdInlinesToPlate(cell.children), isHeader))
...
export function htmlToPlate(html: string): PlateValue {
  const tree = unified().use(rehypeParse, { fragment: true }).parse(html) as unknown as HastRoot
...
function hastElementToBlocks(el: HastElement): PlateElement[] {
  ...
  if (tag === 'blockquote') {
    ...
    for (const child of el.children) {
      if (child.type === 'element' && (child as HastElement).tagName === 'p') {
        inlineChildren.push(...hastInlinesToPlate((child as HastElement).children))
  ...
  const containers = ['div', 'section', 'article', 'main', 'header', 'footer', 'nav', 'aside', 'body', 'html']
...
function hastListToPlate(el: HastElement, ordered: boolean): PlateElement {
  const items = el.children
    .filter((c): c is HastElement => c.type === 'element' && (c as HastElement).tagName === 'li')
...
function hastTableToPlate(el: HastElement): PlateElement {
  ...
  for (const child of el.children) {
    ...
    if (childEl.tagName === 'thead') {
      ...
      const isHeader = childEl.children.some((c) => c.type === 'element' && (c as HastElement).tagName === 'th')
      rows.push(hastTableRowToPlate(childEl, isHeader))
    }
  }

  return createTable(rows.length > 0 ? rows : [createTableRow([createTableCell([createText('')])])])
}

function hastTableRowToPlate(el: HastElement, isHeader: boolean): PlateElement {
  const cells = el.children
    .filter((c): c is HastElement => c.type === 'element' && ['td', 'th'].includes((c as HastElement).tagName))
    .map((cellEl) => {
      const header = isHeader || cellEl.tagName === 'th'
      return createTableCell(hastInlinesToPlate(cellEl.children), header)
    })
  return createTableRow(cells.length > 0 ? cells : [createTableCell([createText('')])])
}
...
function hastInlinesToPlate(children: HastChild[], marks: HastInlineMarks = {}): (PlateText | PlateElement)[] {
  const result = children.flatMap((child) => hastInlineToPlate(child, marks))
  return result.length > 0 ? result : [createText('')]
}

function hastInlineToPlate(child: HastChild, marks: HastInlineMarks = {}): (PlateText | PlateElement)[] {
  if (child.type === 'text') {
  ...
  switch (el.tagName) {
    case 'strong':
    case 'b':
      return el.children.flatMap((c) => hastInlineToPlate(c, { ...marks, bold: true }))
    case 'em':
    case 'i':
      return el.children.flatMap((c) => hastInlineToPlate(c, { ...marks, italic: true }))
    case 'del':
    case 's':
    case 'strike':
      return el.children.flatMap((c) => hastInlineToPlate(c, { ...marks, strikethrough: true }))
    case 'code':
    ...
    case 'a': {
      ...
      return [createLink(url, children.length > 0 ? children : [createText(url)])]
    }
    case 'img': {
...
function hastExtractText(el: HastElement): string {
  return el.children
    .map((child) => {
      ...
      if (child.type === 'element') return hastExtractText(child as HastElement)
      return ''
    })
    .join('')
}
