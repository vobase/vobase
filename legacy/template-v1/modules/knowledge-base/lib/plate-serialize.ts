...
import { NodeType, type PlateElement, type PlateText, type PlateValue } from './plate-types'
...
type MdInline = MdText | MdStrong | MdEmphasis | MdDelete | MdInlineCode | MdLink | MdImage | MdBreak
...
type MdBlock = MdParagraph | MdHeading | MdBlockquote | MdCode | MdList | MdTable | MdThematicBreak
...
function plateInlineToMdast(node: PlateElement | PlateText): MdInline[] {
  if (isPlateText(node)) {
    ...
    if (t.strikethrough) result = { type: 'delete', children: [result] } as MdDelete
    if (t.italic) result = { type: 'emphasis', children: [result] } as MdEmphasis
      ...
      return [
        {
          ...
          url: String(el.url ?? ''),
          title: null,
          children: children.length > 0 ? children : [{ type: 'text', value: String(el.url ?? '') }],
        },
      ]
    }
    case NodeType.IMG:
...
function plateInlinesToMdast(children: (PlateElement | PlateText)[]): MdInline[] {
...
function plateBlockToMdast(node: PlateElement): MdBlock[] {
  switch (node.type) {
    ...
    case NodeType.CODE_BLOCK: {
      const lang = typeof node.lang === 'string' && node.lang ? node.lang : null
      const lines = (node.children as PlateElement[]).map((line) =>
        line.children.map((c) => (isPlateText(c) ? (c as PlateText).text : '')).join(''),
      )
    ...
    default: {
      ...
      const hasContent = inlines.some((i) => i.type !== 'text' || (i as MdText).value !== '')
...
function plateListItemToMdast(li: PlateElement): MdListItem {
  ...
  return {
    type: 'listItem',
    spread: false,
    children: blocks.length > 0 ? blocks : [{ type: 'paragraph', children: [{ type: 'text', value: '' }] }],
  }
}
...
function plateTableToMdast(node: PlateElement): MdTable {
  const rows = (node.children as PlateElement[]).map((row) => {
    const cells = (row.children as PlateElement[]).map((cell) => {
      ...
      const inline = para ? plateInlinesToMdast(para.children) : ([{ type: 'text', value: '' }] as MdInline[])
