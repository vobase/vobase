...
function makeTableRows(count: number, cols = 3, withHeader = true) {
  const rows = []
  if (withHeader) {
    rows.push(createTableRow(Array.from({ length: cols }, (_, c) => createTableCell([createText(`H${c + 1}`)], true))))
  }
  for (let r = 0; r < count; r++) {
    rows.push(
      createTableRow(Array.from({ length: cols }, (_, c) => createTableCell([createText(`R${r + 1}C${c + 1}`)]))),
    )
  }
  return rows
}
...
function makeListItems(count: number, tokenEach: number) {
  return Array.from({ length: count }, (_, i) => createListItem([createText(`Item ${i + 1} ${makeText(tokenEach)}`)]))
}
...
describe('blockChunk — tables', () => {
  it('table is never split with adjacent content', () => {
    ...
    const tableChunks = chunks.filter((c) => c.blocks.some((b) => b.type === 'table'))
  ...
  it('large table (> 1000 tokens) splits into row groups of ~50 rows', () => {
    ...
    // Header row
    rows.push(
      createTableRow(Array.from({ length: colCount }, (_, c) => createTableCell([createText(`Col${c + 1}`)], true))),
    )
    // 120 data rows × 60 tokens each = well over 1000 tokens
    for (let r = 0; r < 120; r++) {
      rows.push(createTableRow(Array.from({ length: colCount }, () => createTableCell([createText(makeText(20))]))))
    }
  ...
  it('large table chunks each contain the header row', () => {
    ...
    for (let r = 0; r < 120; r++) {
      rows.push(
        createTableRow([createTableCell([createText(makeText(30))]), createTableCell([createText(makeText(30))])]),
      )
    }
    const table = createTable(rows)
    const chunks = blockChunk([table])
...
describe('blockChunk — code blocks', () => {
  it('code block is never split', () => {
    const code = createCodeBlock(
      Array.from({ length: 200 }, (_, i) => `const x${i} = ${i}; // some comment here`),
      'typescript',
    )
...
describe('blockChunk — headings', () => {
  it('heading always starts a new chunk', () => {
    const value = [para(20), heading(2, 'Section A'), para(20), heading(2, 'Section B'), para(20)]
    const chunks = blockChunk(value)
    ...
    for (const chunk of chunks) {
      ...
      for (let i = 1; i < chunk.blocks.length; i++) {
        expect(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']).not.toContain(chunk.blocks[i].type)
      }
    }
  })
  ...
  it('heading with enough following content splits into multiple chunks', () => {
    const value = [heading(2, 'Big Section'), ...Array.from({ length: 50 }, () => para(15))]
    const chunks = blockChunk(value)
    expect(chunks.length).toBeGreaterThan(1)
...
describe('blockChunk — heading context prefix', () => {
  it('non-heading chunk carries last heading as prefix', () => {
    ...
    const nonHeadingChunk = chunks.find((c) => !['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].includes(c.blocks[0].type))
  ...
  it('code block chunk carries last heading as prefix', () => {
    const value = [heading(3, 'Implementation'), createCodeBlock(['const x = 1;'])]
    const chunks = blockChunk(value)
    const codeChunk = chunks.find((c) => c.blocks[0].type === 'code_block')
    expect(codeChunk).toBeDefined()
  ...
  it('list does not split mid-nested-item', () => {
    ...
    const nestedItems = Array.from({ length: 20 }, (_, i) => {
      const innerList = createList(
        false,
        Array.from({ length: 5 }, (_, j) => createListItem([createText(`${makeText(8)} nested ${i}-${j}`)])),
      )
      const li = {
        type: 'li' as const,
        children: [{ type: 'lic' as const, children: [createText(`Top ${i}`)] }, innerList],
      }
      return li
    })
