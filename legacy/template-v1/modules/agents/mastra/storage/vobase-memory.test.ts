...
function createMockDelegate() {
  ...
  return {
    ...
    delegate: {
      ...
      saveThread: mock(async ({ thread }: { thread: StorageThreadType }) => thread),
      updateThread: mock(
      ...
      saveMessages: mock(async ({ messages: msgs }: { messages: unknown[] }) => ({
        messages: msgs,
      })),
...
describe('VobaseMemoryStorage', () => {
  ...
  describe('messages', () => {
    it('saveMessages persists with mastraContent and plain text', async () => {
      ...
      // Verify the raw rows
      const rows = await db.select().from(messages).where(eq(messages.conversationId, testConversationId))
  ...
  describe('legacy message handling', () => {
    it('listMessages synthesizes MastraMessageContentV2 for rows without mastraContent', async () => {
      ...
      expect(msg.content.parts).toEqual([{ type: 'text', text: 'Legacy text message' }])
    })
  })
})
...
describe('VobaseMemoryStorage — agent-thread routing', () => {
  ...
  it('saveThread with agent-prefixed ID delegates to omDelegate, NOT conversations table', async () => {
    ...
    const rows = await agentDb.select().from(conversations).where(eq(conversations.id, 'agent-booking-contact-001'))
  ...
  it('saveMessages with agent threadId delegates to omDelegate, NOT messages table', async () => {
    ...
    const rows = await agentDb.select().from(messages).where(eq(messages.id, 'agent-msg-1'))
    expect(rows).toHaveLength(0)
  })
  ...
  it('saveMessages with mixed threadIds splits between delegate and conversations', async () => {
    ...
    const rows = await agentDb.select().from(messages).where(eq(messages.id, 'conv-mixed-1'))
    expect(rows).toHaveLength(1)

    // Agent message NOT in messages table
    const agentRows = await agentDb.select().from(messages).where(eq(messages.id, 'agent-mixed-1'))
