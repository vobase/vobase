import { Actions, Button, Card, CardText } from '@modules/messaging/lib/card-serialization'
...
import { createActivityMessage, insertMessage } from '@modules/messaging/lib/messages'
...
const reply: CommandHandler = async (positional, _flags, ctx) => {
  ...
  const check = await verifyConversationAccess(ctx.deps, ctx.conversationId, ctx.contactId)
...
const card: CommandHandler = async (positional, flags, ctx) => {
  ...
  if (!body) return err('Usage: vobase card <body> [--title <title>] [--buttons "a,b,c"]')
  ...
  if (body.length > constraints.maxBodyLength) {
    return err(`Body exceeds ${constraints.name} limit of ${constraints.maxBodyLength} chars (got ${body.length}).`)
  }
  ...
  if (constraints.maxButtons !== null && buttonLabels.length > constraints.maxButtons) {
    return err(`${constraints.name} allows max ${constraints.maxButtons} buttons, got ${buttonLabels.length}.`)
  }
  ...
  const actionButtons = buttonLabels.map((label, _i) => Button({ id: `chat:${JSON.stringify(label)}`, label }))

  const cardElement = Card({
    ...(flags.title ? { title: flags.title } : {}),
    children: [CardText(body), ...(actionButtons.length > 0 ? [Actions(actionButtons)] : [])],
  })
...
const reassign: CommandHandler = async (positional, flags, ctx) => {
  ...
  if (!spec) return err('Usage: vobase reassign <type:value> [--summary <reason>]')

  const target = parseTargetSpec(spec)
  if (!target) return err(`Invalid target spec: "${spec}". Use role:name, user:id, or agent:id.`)

  const resolvedAssignee = await resolveTarget(ctx.db, target)
  if (!resolvedAssignee) return err(`Could not resolve target: ${target.type}=${target.value}`)
...
const mention: CommandHandler = async (positional, _flags, ctx) => {
  ...
  const target = parseTargetSpec(spec)
  if (!target) return err(`Invalid target spec: "${spec}". Use role:name or user:id.`)

  const targetId = await resolveTarget(ctx.db, target)
  if (!targetId) return err(`Could not resolve target: ${target.type}=${target.value}`)
...
const draft: CommandHandler = async (positional, flags, ctx) => {
  ...
  if (!content) return err('Usage: vobase draft <message> [--reviewer <type:value>]')
  ...
  if (flags.reviewer) {
    ...
    if (!resolved) return err(`Could not resolve reviewer: ${target.type}=${target.value}`)
    reviewerId = resolved
  }
  ...
  await insertMessage(ctx.db, ctx.deps.realtime, {
    ...
    senderType: 'agent',
    private: true,
    ...(reviewerId ? { mentions: [{ targetId: reviewerId, targetType: 'user' as const }] } : {}),
  })
...
const topic: CommandHandler = async (positional, flags, ctx) => {
  ...
  if (!summary) return err('Usage: vobase topic <summary> [--next <next topic>]')
...
const remind: CommandHandler = async (positional, flags, ctx) => {
  ...
  if (!contactId || !message) {
    return err('Usage: vobase remind <contactId> <message> --channel <whatsapp|email>')
  }
...
const followUp: CommandHandler = async (positional, flags, ctx) => {
  ...
  if (!delayStr) return err('Usage: vobase follow-up <delaySeconds> [--reason <reason>]')

  const delaySeconds = Number.parseInt(delayStr, 10)
  if (Number.isNaN(delaySeconds) || delaySeconds < 60 || delaySeconds > 86400 * 7) {
