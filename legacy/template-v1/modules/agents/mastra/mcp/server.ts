 ...
 *
 * Mounted at /api/agents/mcp — SEPARATE from core's /mcp discovery endpoint.
 */
export function createAiMcpHandler(db: VobaseDb): (req: Request) => Promise<Response> {
  return async (req: Request) => {
    ...
    server.registerTool(
      ...
      {
        description: 'Search the knowledge base for relevant documents using hybrid search (vector + keyword).',
        inputSchema: {
