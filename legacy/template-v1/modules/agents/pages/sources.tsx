    ...
    <div className="p-6 space-y-6">
      ...
      {showAdd && (
        <Card className="mb-6">
          <CardContent className="space-y-3">
            ...
            {newType === 'crawl' && (
              <Input placeholder="URL to crawl" value={newUrl} onChange={(e) => setNewUrl(e.target.value)} />
            )}
      ...
      {!isLoading && sources && sources.length === 0 && !showAdd && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Add a source to start syncing documents automatically.</p>
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <Status variant={sourceStatusVariant(source.status)}>
                      <StatusIndicator />
                      <StatusLabel className="capitalize">{source.status}</StatusLabel>
                    </Status>
                  ...
                  <div className="flex items-center gap-2 mt-1.5 ml-4">
                    ...
                    {source.lastSyncAt && (
                      <span className="text-xs text-muted-foreground flex items-center gap-1">
                        &middot; synced <RelativeTimeCard date={source.lastSyncAt} />
                      </span>
                    )}
                  </div>
                </div>
