...
import { createMastraBashTool, createWorkspace } from '../mastra/workspace/create-workspace'
import { formatContent, formatSender } from '../mastra/workspace/materialize-messages'
import { buildWakeMessage, type WakeTrigger } from '../mastra/workspace/wake-message'
...
const wakeAgentSchema = z.object({
  ...
  trigger: z.enum(['inbound_message', 'scheduled_followup', 'supervisor', 'manual']),
 ...
 */
async function fetchNewMessages(deps: ReturnType<typeof getModuleDeps>, conversationId: string) {
...
export const agentWakeJob = defineJob('agents:agent-wake', async (data) => {
  const { agentId, contactId, conversationId, trigger, payload } = wakeAgentSchema.parse(data)
  ...
  if (trigger === 'inbound_message') {
    ...
    if (latest && latest.messageType !== 'activity' && latest.senderType !== 'contact') {
  ...
  const lockRows = (await db.execute(sql`SELECT pg_try_advisory_lock(${lockKey}) AS acquired`)) as unknown as Array<{
    acquired: boolean
  }>
  if (!lockRows[0]?.acquired) {
    logger.info('[agent-wake] Skipping — another wake is running for this conversation', { conversationId })
    return
  }
...
async function runAgentWake(params: {
  ...
  const { agentId, contactId, conversationId, trigger, payload, deps, db, start, abortSignal, onSideEffect } = params
  ...
  const [agentDef] = await db
    ...
    .where(and(eq(agentDefinitions.id, agentId), eq(agentDefinitions.enabled, true)))
  if (!agentDef) {
  ...
  try {
    ...
    scoreConversation({ db, conversationId, agentId, wakeStart }).catch((err) => {
