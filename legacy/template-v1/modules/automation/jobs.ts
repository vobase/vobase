...
export const taskTimeoutJob = defineJob('automation:task-timeout', async () => {
  ...
  const timedOut = await db
    ...
    .where(
      and(
        ...
        lt(sql`${automationTasks.updatedAt} + (${automationTasks.timeoutMinutes} * interval '1 minute')`, sql`now()`),
      ),
    )
  ...
  await db.update(automationTasks).set({ status: 'timeout' }).where(inArray(automationTasks.id, ids))
 ...
 */
export const sessionCleanupJob = defineJob('automation:session-cleanup', async () => {
  const db = getModuleDb()
  ...
  await db.update(automationSessions).set({ status: 'disconnected' }).where(inArray(automationSessions.id, sessionIds))

  for (const id of sessionIds) {
    await unassignOrphanedTasks(id)
  }

  logger.info('[automation] Session cleanup job: disconnected stale sessions', {
