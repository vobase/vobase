...
describe('automation/tasks', () => {
  ...
  describe('completeTask', () => {
    it('only updates tasks in executing state', async () => {
      ...
      const updateCall = (deps.db.update as ReturnType<typeof mock>).mock.calls[0]
  ...
  describe('claimNextTask', () => {
    it('returns null when no tasks available', async () => {
      ...
      ;(deps.db as unknown as { execute: ReturnType<typeof mock> }).execute = mock(async () => ({ rows: [] }))
    ...
    it('returns the claimed task and notifies', async () => {
      ...
      ;(deps.db as unknown as { execute: ReturnType<typeof mock> }).execute = mock(async () => ({
        rows: [{ id: 'task_1' }],
      }))
      ...
      ;(deps.db as unknown as { select: ReturnType<typeof mock> }).select = select
