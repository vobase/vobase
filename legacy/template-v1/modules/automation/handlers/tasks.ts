import { forbidden, getCtx, notFound, unauthorized, validation } from '@vobase/core'
...
const taskStatusSchema = z.enum(['pending', 'executing', 'completed', 'failed', 'timeout', 'cancelled']).optional()
...
export const taskHandlers = new Hono()
  ...
  .get('/', async (c) => {
    ...
    const tasks = status
      ...
      : await db.select().from(automationTasks).orderBy(desc(automationTasks.createdAt)).limit(TASK_PAGE_LIMIT)

    // Strip sensitive browser data from list view
    const sanitized = tasks.map(({ domSnapshot, errorMessage, ...rest }) => rest)
  ...
  .get('/:id', async (c) => {
    ...
    const [task] = await db.select().from(automationTasks).where(eq(automationTasks.id, id)).limit(1)
  ...
  .post('/:id/cancel', async (c) => {
    ...
    if (task.assignedTo && task.assignedTo !== ctx.user.id && ctx.user.role !== 'admin') {
