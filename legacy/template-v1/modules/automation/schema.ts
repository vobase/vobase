...
export const automationSessions = automationPgSchema.table(
  ...
  (table) => [
    ...
    check('sessions_status_check', sql`status IN ('pairing', 'active', 'disconnected', 'expired')`),
  ],
)
...
export const automationTasks = automationPgSchema.table(
  ...
  (table) => [
    ...
    check('tasks_status_check', sql`status IN ('pending', 'executing', 'completed', 'failed', 'timeout', 'cancelled')`),
    check('tasks_requested_by_check', sql`requested_by IN ('ai', 'staff', 'system')`),
  ],
)
...
export const automationPairingCodes = automationPgSchema.table(
  ...
  (table) => [
    ...
    check('pairing_codes_status_check', sql`status IN ('active', 'used', 'expired')`),
  ],
)
