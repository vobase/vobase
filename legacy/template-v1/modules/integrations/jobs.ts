 ...
 */
export const whatsappSetupJob = defineJob('integrations:whatsapp-setup', async (data: unknown) => {
  ...
  if (baseUrl) {
    ...
    subBody.verify_token = process.env.META_WEBHOOK_VERIFY_TOKEN ?? 'vobase-webhook-verify'
    logger.info('WhatsApp setup job: setting WABA callback override', {
      webhookUrl,
    })
  }

  const subRes = await http.fetch(`${META_GRAPH_API}/${wabaId}/subscribed_apps`, {
    method: 'POST',
    headers: {
  ...
  const regRes = await http.fetch(`${META_GRAPH_API}/${phoneNumberId}/register`, {
    method: 'POST',
