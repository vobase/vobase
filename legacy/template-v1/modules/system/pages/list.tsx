...
function SystemDashboardPage(_: Readonly<SystemDashboardPageProps>) {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 lg:p-10">
      <PageHeader title="Operations" description="System health and registered modules" />
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Stat>
          ...
          <StatValue>
            {infoQuery.isPending ? '—' : infoQuery.isError ? 'Unavailable' : (infoQuery.data?.version ?? '—')}
          </StatValue>
        ...
        <Stat>
          ...
          <StatValue>
            {healthQuery.isPending ? '—' : healthQuery.isError ? 'Unavailable' : (healthQuery.data?.status ?? '—')}
          </StatValue>
        ...
        <Stat>
          ...
          <StatValue>
            {healthQuery.isPending ? '—' : healthQuery.isError ? 'Unavailable' : (healthQuery.data?.db ?? '—')}
          </StatValue>
        ...
        <Stat>
          ...
          <StatValue>{infoQuery.isPending ? '—' : infoQuery.isError ? 'Unavailable' : modules.length}</StatValue>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm font-medium">Registered Modules</CardTitle>
        </CardHeader>
        <CardContent>
          {infoQuery.isPending ? (
            ...
            <Empty>
              <EmptyHeader>
                ...
                <EmptyDescription>No modules reported by the API.</EmptyDescription>
              </EmptyHeader>
            </Empty>
          )}
        </CardContent>
