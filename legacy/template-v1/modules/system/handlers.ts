import { auditLog, getCtx, recordAudits, sequences, unauthorized } from '@vobase/core'
...
const systemRoutes = new Hono({ strict: false })
  ...
  .get('/audit-log', async (c) => {
    ...
    const nextCursor = hasMore && lastEntry?.createdAt instanceof Date ? String(lastEntry.createdAt.getTime()) : null
  ...
  .get('/sequences', async (c) => {
    ...
    const values = await db.select().from(sequences).orderBy(desc(sequences.updatedAt))
