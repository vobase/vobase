...
export function KbCurationOverlay({ messageId }: KbCurationOverlayProps) {
  const isSelected = useStaffChatStore((s) => s.kbSelectedMessages.has(messageId))
  const toggleKbMessage = useStaffChatStore((s) => s.toggleKbMessage)

  return (
    <div className="absolute -left-7 top-1/2 -translate-y-1/2">
      <Checkbox checked={isSelected} onCheckedChange={() => toggleKbMessage(messageId)} className="h-4 w-4" />
    </div>
  )
}
...
export function KbCurationToggle() {
  ...
  return (
    <Button variant={isActive ? 'secondary' : 'ghost'} size="sm" onClick={toggle} className="gap-1.5">
...
export function KbCurationBar() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Added ${selectedMessages.size} message${selectedMessages.size > 1 ? 's' : ''} to Knowledge Base`)
        clearKbSelection()
      } else {
