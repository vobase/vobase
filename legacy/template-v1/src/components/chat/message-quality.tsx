...
export function MessageQualityIndicator({ group }: { group: MessageScoreGroup }) {
  if (group.scores.length === 0) return null

  const avg = group.scores.reduce((sum, s) => sum + s.score, 0) / group.scores.length
    ...
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <span className={cn('ml-0.5 text-[10px] font-medium', scoreTextColor(avg))}>{Math.round(avg * 100)}%</span>
        </button>
      </PopoverTrigger>
      <PopoverContent align="start" className="w-64 p-3 max-h-96 overflow-y-auto">
        <p className="text-xs font-medium text-foreground mb-2">Quality Scores</p>
        <div className="space-y-3">
          {group.scores.map((s) => (
            <div key={s.scorerId} className="flex flex-col gap-0.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-muted-foreground">{formatScorerLabel(s.scorerId)}</span>
                <span className={cn('text-xs font-medium', scoreTextColor(s.score))}>{Math.round(s.score * 100)}%</span>
              </div>
