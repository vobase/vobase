...
import { buildCorpus, type Completions, type Corpus, getCompletions } from '@/lib/autocomplete'
...
export function useAutocomplete(input: string, config: AutocompleteConfig): AutocompleteState {
  const { seed, categories, suggestionsUrl, cacheKey = 'autocomplete_corpus_v1' } = config
  ...
  useEffect(() => {
    ...
    async function hydrate() {
      ...
      try {
        ...
        try {
          sessionStorage.setItem(cacheKey, JSON.stringify({ suggestions, fetchedAt: Date.now() }))
        } catch {
          /* storage full */
        }
        if (!cancelled) setCorpus(buildCorpus(seed, categories, suggestions))
  ...
  const { ghost, ghostFull, suggestions } =
    dismissed || !input.trim() ? { ghost: null, ghostFull: null, suggestions: [] } : getCompletions(corpus, input, 8)
