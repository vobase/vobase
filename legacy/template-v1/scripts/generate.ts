...
function buildRoutesSource(moduleNames: string[]): string {
  ...
  const appChildren = ["    route('/', 'home.tsx'),", ...moduleRoutes, ...settingsRoutes]
...
async function checkNavigationCoverage(cwd: string, modulesWithPages: string[]): Promise<void> {
  ...
  for (const name of modulesWithPages) {
    ...
    if (!pattern.test(navSource)) {
      console.warn(`\u26a0 Module "${name}" has pages/ but no navigation entry in src/constants/navigation.ts`)
    }
  }
}
