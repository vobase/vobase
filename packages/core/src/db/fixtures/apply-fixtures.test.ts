...
describe('applyFixtures', () => {
  ...
  it('should execute SQL from current.sql', () => {
    ...
    const result = db.query('SELECT COUNT(*) as count FROM test_table').get() as { count: number };
  ...
  it('should parse and apply --!include directives', async () => {
    ...
    await Bun.write(resolve(fixtureDir, 'base.sql'), 'CREATE TABLE fixture_test (id INTEGER PRIMARY KEY);\n');
    await Bun.write(
    ...
    const result = db.query('SELECT id FROM fixture_test WHERE id = 42').get() as { id: number };
  ...
  it('should handle multiple includes in order', async () => {
    ...
    await Bun.write(resolve(fixtureDir, 'first.sql'), 'CREATE TABLE first (id INTEGER);\n');
    await Bun.write(resolve(fixtureDir, 'second.sql'), 'CREATE TABLE second (id INTEGER);\n');
    await Bun.write(
      resolve(fixtureDir, 'current.sql'),
      '--!include first.sql\n--!include second.sql\n'
    );
    ...
    // Verify both exist
    const tables = db.query("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%'").all();
