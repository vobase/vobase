  ...
  from: (table: unknown) => {
    where: (cond: unknown) => {
      orderBy: (...cols: unknown[]) => {
        limit: (
          n: number,
        ) => Promise<Array<{ type: string; wakeId: string | null }>>;
      };
    };
  };
};
...
export interface JournalService {
  append<E extends JournalEventLike>(
    input: JournalAppendInput<E>,
    tx?: Tx,
  ): Promise<void>;
...
export function createJournalService(deps: JournalServiceDeps): JournalService {
  ...
  async function append<E extends JournalEventLike>(
    input: JournalAppendInput<E>,
    tx?: Tx,
  ): Promise<void> {
  ...
  async function getLastWakeTail(
    conversationId: string,
    ...
    const hasTerminalAfter = wakeTypes
      .slice(lastToolEndIdx + 1)
      .some((t) => TERMINAL.has(t));
  ...
  async function getLatestTurnIndex(
    ...
    const handle =
      (tx as { select: DbHandle['select'] } | undefined) ?? db;
...
function current(): JournalService {
  if (!_currentJournalService) {
    throw new Error(
      'harness/journal: service not installed — call installJournalService() during boot',
    );
  }
...
export async function append<E extends JournalEventLike>(
  input: JournalAppendInput<E>,
  tx?: Tx,
): Promise<void> {
...
export async function getLastWakeTail(
  conversationId: string,
): Promise<{ interrupted: boolean }> {
  return current().getLastWakeTail(conversationId);
}

export async function getLatestTurnIndex(
  conversationId: string,
  tx?: Tx,
): Promise<number> {
