  ...
  server = Bun.serve({
    ...
    async fetch(req) {
      ...
      if (url.pathname === '/slow') {
        return new Promise((resolve) => setTimeout(() => resolve(Response.json({ done: true })), 5000))
      }
...
describe('body serialization', () => {
  test('passes string body through without JSON.stringify', async () => {
    const client = createHttpClient({ baseUrl })
    const res = await client.post<{ body: string; contentType: string | null }>('/echo-raw', 'raw string body')
  ...
  test('passes Blob body through without JSON.stringify', async () => {
    ...
    const res = await client.post<{ body: string; contentType: string | null }>('/echo-raw', blob)
  ...
  test('passes URLSearchParams body through without JSON.stringify', async () => {
    ...
    const res = await client.post<{ body: string; contentType: string | null }>('/echo-raw', params)
