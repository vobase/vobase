...
export interface Scheduler {
  ...
  send(name: string, data: unknown, options?: JobOptions): Promise<string | null>
...
export async function createScheduler(options?: SchedulerOptions): Promise<Scheduler> {
  ...
  async function send(name: string, data: unknown, opts?: JobOptions): Promise<string | null> {
    await ensureQueue(name)
    const queueName = toQueueName(name)
    const sendOpts: SendOptions = {}
    if (opts?.singletonKey !== undefined) sendOpts.singletonKey = opts.singletonKey
    if (opts?.retryBackoff !== undefined) sendOpts.retryBackoff = opts.retryBackoff
    ...
    if (opts?.startAfter !== undefined) sendOpts.startAfter = opts.startAfter as string | Date | number
  ...
  return {
    ...
    send,
    async schedule(name: string, cron: string, data?: unknown, opts?: ScheduleOptions): Promise<void> {
      try {
        await ensureQueue(name)
        const queueName = toQueueName(name)
