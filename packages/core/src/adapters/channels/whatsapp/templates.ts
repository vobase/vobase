...
export function createTemplateOperations(
  ...
  async function createTemplate(template: CreateTemplateInput): Promise<{ id: string; status: string }> {
    if (!TEMPLATE_NAME_RE.test(template.name)) {
      throw new Error(`Invalid template name "${template.name}": must match ^[a-z0-9_]+$`)
    }
    ...
    if (bodyText.length > TEMPLATE_MAX_CHARS) {
      throw new Error(`Template body text (${bodyText.length} chars) exceeds maximum of ${TEMPLATE_MAX_CHARS}`)
    }
    const buttonsComponent = template.components.find((c) => c.type === 'BUTTONS')
    if (buttonsComponent) {
      const buttons = buttonsComponent.buttons
      if (Array.isArray(buttons) && buttons.length > TEMPLATE_MAX_BUTTONS) {
        throw new Error(`Template has ${buttons.length} buttons; maximum is ${TEMPLATE_MAX_BUTTONS}`)
      }
    }
  ...
  async function deleteTemplate(name: string): Promise<void> {
    const wabaId = await getWabaId()
    await graphFetch(`/${wabaId}/message_templates?name=${encodeURIComponent(name)}`, { method: 'DELETE' })
  }
