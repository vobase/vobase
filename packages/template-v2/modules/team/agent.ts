/**
 * Agent-facing surfaces for the team module. Materializers only — staff ids are
 * wake-time data, so the factory pattern is used.
 *
 * `/staff/<staffId>/profile.md` (RO identity card) and `/staff/<staffId>/MEMORY.md`
 * (agent-writable per-(agent, staff) memory, backed by `agents.agent_staff_memory`).
 *
...
export { buildTeamMaterializers as buildMaterializers }
