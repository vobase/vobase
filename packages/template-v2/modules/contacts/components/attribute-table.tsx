/**
 * AttributeTable — fill-in grid for a contact's custom attributes.
 * Pulls the org-wide definitions, merges with the contact's current values,
 * and persists edits via `useSetContactAttributes`. Definition management
 * lives on the dedicated `/contacts/attributes` page.
 */
...
export function AttributeTable({ contactId, values }: Props) {
  ...
  const setAttrs = useSetContactAttributes(contactId)
  ...
  if (defs.length === 0) {
    return (
      <Empty>
        ...
        <EmptyDescription>
          Create shared fields once on the attributes page, then fill them in for every contact.
        </EmptyDescription>
        <div className="mt-3">
          <Button asChild size="sm" variant="outline">
            <Link to="/contacts/attributes">Manage attributes</Link>
          </Button>
        </div>
      </Empty>
    )
