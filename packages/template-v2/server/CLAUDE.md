...
Path alias: `@server/*` → `server/*`. Cross-module access goes directly to the other module's service: `import { appendTextMessage } from '@modules/inbox/service/messages'`. The `messages` / `conversation_events` tables have a CI-enforced one-write-path guard (`checkJournalWriteAuthority`); other tables are open.
