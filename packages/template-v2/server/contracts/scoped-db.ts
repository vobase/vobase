 ...
 * module services already use, without re-deriving organization-scoping at every
 * call site.
 *
 * Design:
 * - `ScopedDb` is a named alias over `PostgresJsDatabase<Schema>`. The alias
 *   records the intent ("this handle carries organization-aware call sites") without
 ...
 * - `OrganizationScope` names the organization-filter carrier shape. The runtime observer/
 *   mutator contexts already expose `organizationId` at the top level; consumers
 *   thread `ctx.organizationId` into explicit `where organizationId = ?` clauses rather
...
/**
 * Organization-filter helper shape. Documented here so downstream lanes can
 * consume the same carrier name when threading `organizationId` through service
