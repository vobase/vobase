...
**Cross-module access.** Import the service directly: `import { appendTextMessage } from '@modules/inbox/service/messages'`. Domain entity types (`Conversation`, `Message`, `Contact`, etc.) live in each module's `schema.ts`. Service interface types (`InboxPort`, `AgentsPort`, `FilesService`) live in each module's `service/types.ts` or `service/files.ts` — these are the wiring contracts for `PluginContext.ports`, wired by `server/dev/dev-ports.ts` for dev + test.
