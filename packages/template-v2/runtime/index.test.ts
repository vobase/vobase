/**
 * runtime.test.ts — `ScopedDb` refines Drizzle's `PostgresJsDatabase<Schema>`
...
import type { OrganizationScope, Schema, ScopedDb } from './index'
...
describe('ScopedDb contract', () => {
  ...
  it('OrganizationScope names the organization-filter carrier shape', () => {
    ...
    type _OrganizationScope = AssertTrue<AssertEqual<keyof OrganizationScope, 'organizationId'>>
    const _os: _OrganizationScope = true
    expect(_os).toBe(true)
  })
})
