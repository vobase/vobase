...
const monorepoRoot = templateMode ? resolve(dirname(import.meta.path), '..', '..') : null
...
if (isCurrent) {
  try {
    ...
    if (status) {
      console.error('Error: Current directory has uncommitted changes. Commit or stash them first.')
      process.exit(1)
    }
  } catch {
...
{
  const pluginsSrc = monorepoRoot ? resolve(monorepoRoot, '.biome/plugins') : null
...
const biomePath = resolve(dest, 'biome.json')
{
  ...
  if (rootBiomeSrc && existsSync(rootBiomeSrc)) {
    ...
    const url = 'https://raw.githubusercontent.com/vobase/vobase/main/biome.json'
    const res = await fetch(url)
    if (res.ok) {
      rootConfig = JSON.parse(await res.text())
    } else {
      console.warn(`${dim('!')} Could not fetch root biome.json (${res.status}) — generating minimal config`)
    }
  }
  ...
  if (existsSync(templateBiomePath)) {
    try {
      const templateConfig = JSON.parse(readFileSync(templateBiomePath, 'utf8'))
