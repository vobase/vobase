...
function serializeInlineNode(node: PlateElement | PlateText): string {
  ...
  const inner = (node.children as (PlateElement | PlateText)[]).map(serializeInlineNode).join('')
  if (node.type === NodeType.A) return `[${inner}](${String(node.url ?? '')})`
  return inner
}

function serializeListItem(li: PlateElement, bullet: string, depth: number): string {
  ...
  for (const child of li.children as (PlateElement | PlateText)[]) {
    if ('text' in child) {
      ...
      if (el.type === NodeType.LIC) {
        textContent = (el.children as (PlateElement | PlateText)[]).map(serializeInlineNode).join('')
...
function serializeTableNode(table: PlateElement): string {
  ...
  for (const row of rows) {
    ...
    const cellTexts = cells.map((c) => {
      ...
      return children
        .map((p) =>
          'text' in p ? serializeInlineNode(p) : (p as PlateElement).children.map(serializeInlineNode).join(''),
        )
        .join('')
    })
...
function serializeBlock(block: PlateElement, depth = 0): string {
  ...
  if (block.type === NodeType.P) {
    return (block.children as (PlateElement | PlateText)[]).map(serializeInlineNode).join('')
  }
  if (block.type === NodeType.BLOCKQUOTE) {
    return (block.children as (PlateElement | PlateText)[])
      .map((c) => `> ${'text' in c ? (c as PlateText).text : serializeBlock(c as PlateElement)}`)
      .join('\n')
  }
  ...
  return (block.children as (PlateElement | PlateText)[])
    .map((c) => ('text' in c ? (c as PlateText).text : serializeBlock(c as PlateElement, depth)))
    .join('')
}
...
function getHeaderRows(rows: PlateElement[]): PlateElement[] {
  return rows.filter((row) => (row.children as PlateElement[]).some((c) => c.type === NodeType.TH))
}
...
function splitLargeList(
  ...
  for (const item of items) {
    ...
    const testContent = headingPrefix ? `${headingPrefix}\n\n${testRaw}` : testRaw
 ...
 */
export function blockChunk(value: PlateValue, options?: ChunkOptions): BlockChunk[] {
  ...
  for (let i = 0; i < value.length; i++) {
    ...
    if (isList(block)) {
      ...
      if (currentBlocks.length > 0) {
        ...
        const candidateContent = candidatePrefix ? `${candidatePrefix}\n\n${candidateRaw}` : candidateRaw

        if (estimateTokens(candidateContent) > maxTokens) {
          flushCurrent(i - 1)
          const listRaw = plateToMarkdown([block])
          const listContent = headingPfx ? `${headingPfx}\n\n${listRaw}` : listRaw
          if (estimateTokens(listContent) > maxTokens) {
            const splitChunks = splitLargeList(block, i, headingPfx, maxTokens, result.length)
    ...
    if (currentBlocks.length > 0) {
      const candidateRaw = plateToMarkdown([...currentBlocks, block])
      const prefix = getHeadingPrefix(currentBlocks[0])
      const candidateContent = prefix ? `${prefix}\n\n${candidateRaw}` : candidateRaw
      if (estimateTokens(candidateContent) > maxTokens) {
        flushCurrent(i - 1)
        currentBlocks = [block]
      } else {
