...
export async function hybridSearch(db: VobaseDb, query: string, options?: SearchOptions): Promise<SearchResult[]> {
  ...
  if (query.trim()) {
    try {
      keywordResults = await db
        ...
        .from(kbChunks)
        .where(sql`${kbChunks.searchVector} @@ websearch_to_tsquery('english', ${query})`)
        .orderBy(sql`ts_rank(${kbChunks.searchVector}, websearch_to_tsquery('english', ${query})) DESC`)
        .limit(fetchCount)
    } catch {
  ...
  for (const { id, score } of topCandidates) {
    const chunk = (await db.select().from(kbChunks).where(eq(kbChunks.id, id)))[0]
    if (!chunk) continue
