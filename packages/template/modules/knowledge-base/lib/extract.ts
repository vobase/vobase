...
let pdfiumPromise: Promise<Awaited<ReturnType<typeof PDFiumLibrary.init>>> | null = null
...
function hasOcrProvider(): boolean {
  return !!(process.env.BIFROST_API_KEY || process.env.GOOGLE_GENERATIVE_AI_API_KEY)
}
...
export async function encodeToJpeg(input: sharp.Sharp): Promise<{ data: Buffer; width: number; height: number }> {
  const meta = await input.metadata()
  if ((meta.width && meta.width > MAX_IMAGE_DIMENSION) || (meta.height && meta.height > MAX_IMAGE_DIMENSION)) {
    input = input.resize({
  ...
  const { data, info } = await input.jpeg({ quality: JPEG_QUALITY }).toBuffer({ resolveWithObject: true })
...
export async function extractDocument(filePath: string, mimeType: string): Promise<ExtractionResult> {
  ...
  if (mimeType === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') return extractXlsx(buffer)
...
function batchPages(pages: OcrImage[]): OcrImage[][] {
  ...
  for (const page of pages) {
    if (current.length > 0 && currentSize + page.data.byteLength > MAX_BATCH_BYTES) {
...
async function extractPdf(buffer: ArrayBuffer, sizeMB: string): Promise<ExtractionResult> {
  if (!hasOcrProvider()) {
    logger.warn('[kb] pdf_extract_skip', { reason: 'no API key', sizeMB })
    return {
      value: emptyPlate(),
      status: 'needs_ocr',
      warning: 'PDF extraction requires BIFROST_API_KEY or GOOGLE_GENERATIVE_AI_API_KEY.',
    }
  }
  ...
  try {
    ...
    if (!isReadableText(combined)) {
      ...
      return {
        value: emptyPlate(),
        status: 'needs_ocr',
        warning: 'PDF extraction produced insufficient or unreadable text from Gemini.',
      }
    }
...
async function extractImage(buffer: ArrayBuffer): Promise<ExtractionResult> {
  if (!hasOcrProvider()) {
    ...
    return {
      value: emptyPlate(),
      status: 'needs_ocr',
      warning: 'Image OCR requires BIFROST_API_KEY or GOOGLE_GENERATIVE_AI_API_KEY.',
    }
  }
...
async function extractXlsx(buffer: ArrayBuffer): Promise<ExtractionResult> {
  ...
  for (const name of wb.SheetNames) {
    ...
    const headerRow = createTableRow(header.map((h) => createTableCell([createText(String(h ?? ''))], true)))
    const bodyRows = body.map((row) =>
      createTableRow(header.map((_, i) => createTableCell([createText(String(row[i] ?? ''))]))),
    )
...
async function extractPptx(buffer: ArrayBuffer): Promise<ExtractionResult> {
  ...
  if (hasOcrProvider() && avgCharsPerSlide < 50 && slides.length > 0) {
    try {
      const ocrText = await ocrFile(buffer, 'application/vnd.openxmlformats-officedocument.presentationml.presentation')
      const value = markdownToPlate(ocrText)
      return { value, status: 'ok' }
    } catch {
...
async function ocrBatch(pages: OcrImage[], batchContext: string): Promise<string> {
  ...
  const { text, finishReason } = await generateText({
    ...
    messages: [
      {
        role: 'user',
        content: [...imageParts, { type: 'text' as const, text: buildOcrPrompt(batchContext) }],
      },
    ],
  })
...
async function ocrWithFallback(page: OcrImage, genText: typeof import('ai').generateText): Promise<string> {
