...
const fakeSharpInstance: Record<string, unknown> = {
  ...
  toBuffer: async (opts?: { resolveWithObject?: boolean }) =>
    opts?.resolveWithObject
      ? { data: fakeJpeg, info: { width: 100, height: 100 } }
      : fakeJpeg,
};
mock.module('sharp', () => ({
...
describe('extractDocument()', () => {
  ...
  describe('text formats', () => {
    it('extracts plain text from .txt files', async () => {
      ...
      expect(
        'text' in firstChild && (firstChild as { text: string }).text,
      ).toContain('Hello');
    });
  ...
  describe('XLSX extraction', () => {
    ...
    it('uses TH cells for the header row', async () => {
      ...
      const firstRow = (
        table?.children as { type: string; children: unknown[] }[]
      )[0];
...
describe('round-trip serialization', () => {
  ...
  it('inline code does not combine with bold/italic marks', () => {
    ...
    const codeChild = para.children.find(
      (c) => 'code' in c && (c as { code?: boolean }).code === true,
    );
