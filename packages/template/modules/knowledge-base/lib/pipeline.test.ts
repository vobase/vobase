...
mock.module('../../../lib/embeddings', () => ({
  embedChunks: async (texts: string[]) => texts.map((_, i) => [i * 0.1, 1 - i * 0.1, 0.5, 0.5]),
...
describe('processDocument()', () => {
  ...
  it('marks document as ready after processing', async () => {
    ...
    const [doc] = await db.select().from(kbDocuments).where(eq(kbDocuments.id, 'doc-1'))
  ...
  it('sets chunkCount to 0 for empty content', async () => {
    ...
    const [doc] = await db.select().from(kbDocuments).where(eq(kbDocuments.id, 'doc-2'))
    expect(doc?.status).toBe('ready')
  ...
  it('creates chunks in the database', async () => {
    ...
    await processDocument(db, 'doc-3', textPlate('This is a piece of content that will become a chunk.'))
  ...
  it('stores embeddings inline in kb_chunks', async () => {
    ...
    await processDocument(db, 'doc-4', textPlate('Content for vector embedding.'))
  ...
  it('generates tsvector for full-text search', async () => {
    ...
    await processDocument(db, 'doc-5', textPlate('Searchable full text content here.'))
  ...
  it('assigns chunkIndex starting at 0', async () => {
    ...
    const chunks = await db.select().from(kbChunks).where(eq(kbChunks.documentId, 'doc-6'))
    if (chunks.length > 0) {
  ...
  it('marks document as error on failure', async () => {
    ...
    const [doc] = await db.select().from(kbDocuments).where(eq(kbDocuments.id, 'doc-7'))
  ...
  it('updates document chunkCount correctly', async () => {
    ...
    const [doc] = await db.select().from(kbDocuments).where(eq(kbDocuments.id, 'doc-8'))
  ...
  it('stores content and rawContent as jsonb on the document', async () => {
    ...
    const [doc] = await db.select().from(kbDocuments).where(eq(kbDocuments.id, 'doc-9'))
