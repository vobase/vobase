...
export function createGoogleDriveConnector(
  ...
  return {
    ...
    async *listDocuments(): AsyncGenerator<ExternalDocument> {
      ...
      do {
        const query = config.folderId ? `'${config.folderId}' in parents and trashed = false` : 'trashed = false'
    ...
    async fetchDocument(externalId: string): Promise<DocumentContent> {
      ...
      if (mimeType.startsWith('application/vnd.google-apps.')) {
        ...
        const res = await drive.files.export({ fileId: externalId, mimeType: exportMime }, { responseType: 'text' })
        return { text: String(res.data), metadata: { name: meta.data.name } }
      }

      // Other files: download content
      const res = await drive.files.get({ fileId: externalId, alt: 'media' }, { responseType: 'text' })
      return { text: String(res.data), metadata: { name: meta.data.name } }
    },
  }
}
...
export async function exchangeGoogleCode(
  ...
  const integration = await integrations.connect('google-drive', tokens as Record<string, unknown>, {
    authType: 'oauth2',
