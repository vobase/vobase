...
export const contactsProposeChangeVerb = defineCliVerb({
  ...
  prompt: [
    'Customer-asked profile updates (email, phone, name, segments, attributes) MUST go through this verb — `/contacts/<id>/PROFILE.md` is read-only at the workspace level, so bash-edits will be rejected. Run e.g. `vobase contacts propose-change --id <contactId> --field email --to "new@example.com" --rationale "Customer asked to update on this channel"`. (`--kind field_set` is the default; pass `--kind markdown_patch --body "..."` for prose appends.) For canonical columns use the bare name (`--field phone`, `--field email`, `--field displayName`); the `attributes.<key>` namespace is for tenant-defined fields listed under "## Structured fields" in the contact PROFILE.md (e.g. `--field attributes.vip --to true`, `--field attributes.occasion --to "wedding"`, `--field attributes.lifetime_value --to 1500`).',
    '',
    'ANY fact about the customer that fits a structured field listed in `## Structured fields` MUST go through `field_set` for that field — NOT `markdown_patch` on `memory`. "Mark as VIP" → `--field attributes.vip --to true`. "Occasion is a wedding" → `--field attributes.occasion --to "wedding"`. Only use `markdown_patch` on `memory` for narrative context that has no structured slot (preferences, history, anecdotes).',
    '',
