...
import { chatAssistants, chatMessages, chatThreads } from './schema';
...
chatbotRoutes.get('/assistants/:id', async (c) => {
  ...
  const assistant = await ctx.db
    .select()
    .from(chatAssistants)
    .where(eq(chatAssistants.id, c.req.param('id')))
    .get();
...
chatbotRoutes.put('/assistants/:id', async (c) => {
  ...
  const [assistant] = await ctx.db
    ...
    .where(
      and(
        eq(chatAssistants.id, c.req.param('id')),
        eq(chatAssistants.userId, ctx.user!.id),
      ),
    )
    .returning();
  if (!assistant) throw notFound('Assistant not found');
...
chatbotRoutes.get('/threads/:id', async (c) => {
  ...
  const thread = await ctx.db
    ...
    .where(
      and(
        eq(chatThreads.id, c.req.param('id')),
...
chatbotRoutes.delete('/threads/:id', async (c) => {
  ...
  const thread = await ctx.db
    .select()
    .from(chatThreads)
    .where(and(eq(chatThreads.id, id), eq(chatThreads.userId, ctx.user!.id)))
    .get();
  if (!thread) throw notFound('Thread not found');
