 ...
 * and accepts valid input. Uses a real DB to test sessionCleanupJob's query logic.
 *
...
describe('sendJob schema (conversations:send)', () => {
...
describe('channelReplyJob schema (conversations:channel-reply)', () => {
  it('accepts valid sessionId without inboundContent', () => {
    const result = channelReplyDataSchema.parse({ sessionId: 'sess-1' });
    expect(result.sessionId).toBe('sess-1');
    expect(result.inboundContent).toBeUndefined();
  });

  it('accepts valid sessionId with inboundContent', () => {
...
describe('processInboundJob schema (conversations:process-inbound)', () => {
...
describe('retryMemoryThreadJob schema (conversations:retry-memory-thread)', () => {
  const validInput = {
    sessionId: 'sess-1',
    contactId: 'contact-1',
    agentId: 'booking',
...
// ─── sessionCleanupJob integration ───────────────────────────────────────────
// Test the stale session query logic with a real DB (without mock.module).
...
describe('sessionCleanupJob stale-session query', () => {
  it('finds sessions inactive for 7+ days', async () => {
    ...
    await db
      ...
      .where(eq(sessions.id, 'sess-stale'));
  ...
  it('does not find recently-active sessions', async () => {
    await db.insert(contacts).values({
      id: 'contact-2',
      phone: '+6598765432',
      name: 'Recent',
