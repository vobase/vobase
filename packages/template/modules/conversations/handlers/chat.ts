...
// ─── Public Chat Routes (unauthenticated, visitor-token based) ──────
...
chatHandlers.post('/chat/:endpointId/start', async (c) => {
...
chatHandlers.get(
  '/chat/:endpointId/conversations/:conversationId',
  async (c) => {
    const { db } = getCtx(c);
    const endpointId = c.req.param('endpointId');
...
chatHandlers.post('/chat/:endpointId/stream', async (c) => {
  const { db } = getCtx(c);
  const endpointId = c.req.param('endpointId');
  const visitorToken =
