...
contactsHandlers.get('/search', async (c) => {
  const { db } = getCtx(c);
...
contactsHandlers.get('/:id', async (c) => {
  const { db } = getCtx(c);
...
contactsHandlers.patch('/:id', async (c) => {
  const { db } = getCtx(c);
  const id = c.req.param('id');
