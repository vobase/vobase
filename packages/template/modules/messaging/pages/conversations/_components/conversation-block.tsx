...
import { AssigneeBadge, PriorityBadge, StatusBadge } from '@/components/conversation-badges'
...
export const ConversationBlock = memo(function ConversationBlock({
  ...
  const isTerminal = conversation.status === 'resolved' || conversation.status === 'failed'
  const accentClass = CHANNEL_LINE_ACTIVE[conversation.channelType]

  const visibleMessages = useMemo(() => filterAndSortMessages(messages), [messages])

  // Long-thread truncation
  const exceedsThreshold = visibleMessages.length > LONG_THREAD_THRESHOLD && !showAll
  const displayMessages = exceedsThreshold ? visibleMessages.slice(-LONG_THREAD_PREVIEW) : visibleMessages

  // Collapsed state: last few non-activity messages as text preview
  const collapsedPreview = useMemo(
    () => visibleMessages.filter((m) => m.messageType !== 'activity').slice(-COLLAPSED_PREVIEW_COUNT),
    [visibleMessages],
  )
  ...
  const endLabel = conversation.resolvedAt
    ...
    : null
  const dateRange = endLabel && endLabel !== startLabel ? `${startLabel} – ${endLabel}` : startLabel

  const title = conversation.channelLabel ?? 'Conversation'

  const handleReplyClick = (messageId: string, senderName: string, contentPreview: string) => {
          ...
          {!isTerminal && (
            ...
            >
              ...
              <Button
                ...
                size="sm"
                className={cn('h-7 gap-1.5 text-xs', conversation.onHold && 'text-amber-600 dark:text-amber-400')}
                onClick={() => onUpdateConversation({ onHold: !conversation.onHold })}
              >
                {conversation.onHold ? <PlayIcon className="h-3 w-3" /> : <PauseIcon className="h-3 w-3" />}
              </Button>
              <DropdownMenu>
          ...
          collapsedPreview.length > 0 && (
            ...
            >
              {collapsedPreview.map((msg) => (
                <CollapsedPreviewRow key={msg.id} message={msg} senderMap={senderMap} currentUserId={currentUserId} />
              ))}
            </div>
          )
        )}
...
export const BlockMessageItem = memo(function BlockMessageItem({
  ...
  const isMe = !!currentUserId && message.senderType === 'user' && message.senderId === currentUserId

  if (isMe) {
    return (
      <OutgoingMessage message={message} sender={sender} onRetry={onRetry} onReplyClick={onReplyClick} align="right" />
    )
  }

  if (message.messageType === 'incoming') {
    return <IncomingMessage message={message} sender={sender} onReplyClick={onReplyClick} />
  }

  return (
    <OutgoingMessage message={message} sender={sender} onRetry={onRetry} onReplyClick={onReplyClick} scores={scores} />
  )
})
...
function CollapsedPreviewRow({
  ...
  return (
    <div className="flex items-baseline gap-1.5 text-xs text-muted-foreground">
      <span className="font-medium text-foreground/70 shrink-0">{senderLabel}:</span>
