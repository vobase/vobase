...
function UlElement(props: PlateElementProps) {
  return (
    <PlateElement
      {...pe(props)}
      className="my-1 ml-4 list-disc space-y-0.5 text-sm"
    />
  );
}
...
function OlElement(props: PlateElementProps) {
  return (
    <PlateElement
      {...pe(props)}
      className="my-1 ml-4 list-decimal space-y-0.5 text-sm"
    />
  );
}
...
export const emailEditorPlugins = [
  BasicBlocksPlugin,
  BasicMarksPlugin,
  ListPlugin,
];

export const emailEditorComponents: Record<
  string,
