...
import { EditIcon, InfoIcon, PlusIcon, RefreshCwIcon, SendIcon, TrashIcon } from 'lucide-react'
...
function templateStatusVariant(status: string | null): StatusVariant {
  ...
  if (status === 'REJECTED' || status === 'DISABLED' || status === 'PAUSED') return 'error'
  return 'default'
}
  ...
  return [
    ...
    },
    {
      ...
      filterFn: (row, id, value) =>
        Array.isArray(value) ? value.includes(row.getValue(id)) : row.getValue(id) === value,
      meta: {
        label: 'Category',
        variant: 'multiSelect',
        options: [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (!body) return <span className="text-muted-foreground/40">&mdash;</span>
        return <span className="text-sm text-muted-foreground line-clamp-1 max-w-[300px]">{body}</span>
      },
      enableSorting: false,
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => <RelativeTimeCard date={row.getValue('syncedAt') as string} />,
      meta: { label: 'Updated' },
      enableSorting: true,
    },
    {
...
function TemplateDialog({
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const components: Array<{ type: string; [key: string]: unknown }> = [{ type: 'BODY', text: bodyText }]
      const res = await messagingClient.templates.$post({
        json: { name, language: 'en', category, components },
      })
      if (!res.ok) {
        const err = await res.json().catch(() => null)
        throw new Error((err as Record<string, string> | null)?.message ?? 'Failed to create template')
      }
      return res.json()
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => null)
        throw new Error((err as Record<string, string> | null)?.message ?? 'Failed to update template')
      }
      return res.json()
    },
    onSuccess: () => {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex flex-col gap-4 py-2">
          ...
          {category === 'MARKETING' && (
            <div className="flex items-start gap-2 rounded-md border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800 dark:border-blue-800 dark:bg-blue-950/50 dark:text-blue-300">
              ...
              <span>Unsubscribe button will be added automatically when submitted.</span>
            </div>
          )}
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-xs ${bodyText.length > BODY_MAX_CHARS ? 'text-destructive' : 'text-muted-foreground'}`}
              >
            ...
            <div className="flex items-center gap-2">
              <Button type="button" variant="outline" size="sm" className="h-7 text-xs gap-1" onClick={insertVariable}>
                <PlusIcon className="size-3" />
                Insert Variable{' '}
                <span className="font-mono text-muted-foreground">{`{{${getNextVariableNumber(bodyText)}}}`}</span>
              </Button>
              <span className="text-xs text-muted-foreground">Variables are mapped to CSV columns when sending.</span>
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={isPending || !name || !bodyText || bodyText.length > BODY_MAX_CHARS}
          >
            {isPending ? (isEditing ? 'Saving...' : 'Creating...') : isEditing ? 'Save Changes' : 'Create Draft'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function TemplatesPage() {
  ...
  const submitMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => null)
        throw new Error((err as Record<string, string> | null)?.message ?? 'Failed to submit template')
      }
    ...
    <div className="flex flex-1 flex-col gap-4 p-4 sm:gap-6 sm:p-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Create and manage WhatsApp message templates for broadcasts.</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      {/* Create/Edit dialog */}
      <TemplateDialog open={dialogOpen} onOpenChange={setDialogOpen} editTemplate={editTemplate} />
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will submit <span className="font-mono font-medium text-foreground">{submitTarget?.name}</span> to
              Meta for approval. The template cannot be edited after submission.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {deleteTarget?.externalId ? ' from both your account and Meta.' : '.'}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
