...
import { evaluateDateRelativeRules, evaluateRecurringRules } from './lib/automation-engine'
...
import { broadcasts, channelInstances, conversations, messages } from './schema'
 ...
 */
export const deliverMessageJob = defineJob('messaging:deliver-message', async (data) => {
 ...
 */
export const conversationCleanupJob = defineJob('messaging:conversation-cleanup', async () => {
  const deps = getModuleDeps()
 ...
 */
export const resolvingTimeoutJob = defineJob('messaging:resolving-timeout', async () => {
  const deps = getModuleDeps()
  ...
  const stuck = await deps.db
    ...
    .from(conversations)
    .where(and(eq(conversations.status, 'resolving'), lt(conversations.updatedAt, sixtySecondsAgo)))
 ...
 */
export const processInboundJob = defineJob('messaging:process-inbound', async (data) => {
 ...
 */
export const sessionExpiryJob = defineJob('messaging:session-expiry', async () => {
  const deps = getModuleDeps()
...
const FALLBACK_CAPTION = '(media received — caption unavailable, use analyze_media tool to examine)'
 ...
 */
export const processMediaCaptionJob = defineJob('messaging:process-media-caption', async (data) => {
  ...
  const mediaArray = (contentData.media as Array<{ storageKey?: string; mimeType: string }>) ?? []
 ...
 */
export const channelHealthCheckJob = defineJob('messaging:channel-health-check', async () => {
  const deps = getModuleDeps()
  ...
  await Promise.allSettled(
    instances.map(async (instance) => {
      const adapter = deps.channels.getAdapter(instance.id) ?? deps.channels.getAdapter(instance.type)
 ...
 */
export const broadcastExecuteJob = defineJob('broadcast:execute', async (data) => {
 ...
 */
export const broadcastCheckScheduledJob = defineJob('broadcast:check-scheduled', async () => {
  const deps = getModuleDeps()
  const now = new Date()
  ...
  for (const b of due) {
    await deps.db.update(broadcasts).set({ status: 'sending', startedAt: now }).where(eq(broadcasts.id, b.id))

    await deps.scheduler.add('broadcast:execute', { broadcastId: b.id }, { singletonKey: b.id }).catch((err) => {
 ...
 */
export const broadcastRetryFailedJob = defineJob('broadcast:retry-failed', async (data) => {
  const { broadcastId } = broadcastDataSchema.parse(data)
  await executeBroadcast(broadcastId)
 ...
 */
export const automationEvaluateRecurringJob = defineJob('automation:evaluate-recurring', async () => {
 ...
 */
export const automationEvaluateDateRelativeJob = defineJob('automation:evaluate-date-relative', async () => {
 ...
 */
export const automationAdvanceChasersJob = defineJob('automation:advance-chasers', async () => {
 ...
 */
export const automationRescheduleCheckJob = defineJob('automation:reschedule-check', async () => {
 ...
 */
export const automationExecuteStepJob = defineJob('automation:execute-step', async (data) => {
  const { executionId, stepSequence } = automationExecuteStepDataSchema.parse(data)
