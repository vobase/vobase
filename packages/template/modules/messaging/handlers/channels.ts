...
const createEndpointSchema = z.object({
  ...
  assignmentPattern: z.enum(['direct', 'router', 'workflow']).optional().default('direct'),
  config: z.record(z.string(), z.unknown()).optional(),
})
...
export const channelsHandlers = new Hono()
  ...
  .get('/channels/managed-available', async (c) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        logger.warn('[messaging] Failed to fetch managed channels from platform', {
          status: res.status,
        })
      }
    } catch (err) {
    ...
    const localInstances = await db.select().from(channelInstances).where(eq(channelInstances.source, 'platform'))
  ...
  .post('/channels/managed-connect', async (c) => {
    ...
    if (!agentRow) throw validation({ agentId: `Agent '${body.agentId}' not found` })

    const instanceId = generateId()

    await ctx.db.transaction(async (tx) => {
      // Check inside transaction to prevent race condition
      const existing = await tx.select().from(channelInstances).where(eq(channelInstances.source, 'platform'))
    ...
    ctx.channels.registerAdapter(
      ...
      createWhatsAppAdapter({
        ...
        accessToken: '',
        appSecret: '',
        transport: buildManagedTransport(platformUrl, hmacSecret, tenantId, body.managedChannelId),
      }),
    )
  ...
  .post('/channels/whatsapp/connect', async (c) => {
      ...
      return c.json(
        {
          error: {
            code: 'SERVER_ERROR',
            message: 'META_APP_ID and META_APP_SECRET must be set in environment',
          },
        },
        500,
      )
    ...
    if (body.agentId) {
      const [agentCheck] = await ctx.db
        ...
        .from(agentDefinitions)
        .where(and(eq(agentDefinitions.id, body.agentId), eq(agentDefinitions.enabled, true)))
      if (!agentCheck) throw validation({ agentId: `Agent '${body.agentId}' not found` })
    }

    // Step 1: Exchange authorization code for BISU access token
    // Code expires in ~60 seconds — must be exchanged immediately
    const tokenRes = await ctx.http.fetch(`${META_GRAPH_API}/oauth/access_token`, {
      method: 'POST',
    ...
    if (!wabaId || !phoneNumberId) {
      try {
        const debugRes = await ctx.http.fetch(`${META_GRAPH_API}/debug_token?input_token=${accessToken}`, {
        ...
        if (!phoneNumberId && wabaId) {
          const phoneRes = await ctx.http.fetch(`${META_GRAPH_API}/${wabaId}/phone_numbers`, {
      ...
      return c.json(
        {
          error: {
            code: 'BAD_GATEWAY',
            message: 'Could not determine WABA or phone number. Check your Meta App permissions.',
          },
        },
        502,
      )
    ...
    const instanceId = generateId()
    const label = displayPhoneNumber ? `WhatsApp ${displayPhoneNumber}` : `WhatsApp ${phoneNumberId}`
  ...
  .get('/instances', async (c) => {
    ...
    const rows = await db.select().from(channelInstances).orderBy(desc(channelInstances.createdAt))
  ...
  .patch('/instances/:id', async (c) => {
    ...
    const [existing] = await db.select().from(channelInstances).where(eq(channelInstances.id, instanceId))
  ...
  .delete('/instances/:id', async (c) => {
    ...
    const activeConversations = await db
      ...
      .from(conversations)
      .innerJoin(channelRoutings, eq(conversations.channelRoutingId, channelRoutings.id))
      .where(and(eq(channelRoutings.channelInstanceId, instanceId), eq(conversations.status, 'active')))
      .limit(1)
      ...
      return c.json(
        {
          error: {
            code: 'CONFLICT',
            message: 'This channel has active conversations. Resolve or end them before removing.',
          },
        },
        409,
      )
    ...
    if (isManagedChannel) {
      ...
      if (relatedRoutingIds.length > 0) {
        await db.delete(channelRoutings).where(inArray(channelRoutings.id, relatedRoutingIds))
      }
      await db.update(channelInstances).set({ status: 'disconnected' }).where(eq(channelInstances.id, instanceId))
      ...
      if (relatedRoutingIds.length > 0) {
        ...
        if (relatedConversationIds.length > 0) {
          await db.delete(conversations).where(inArray(conversations.id, relatedConversationIds))
        }
        await db.delete(channelRoutings).where(inArray(channelRoutings.id, relatedRoutingIds))
      }
  ...
  .post('/instances/:id/complete-setup', async (c) => {
    ...
    const [instance] = await db.select().from(channelInstances).where(eq(channelInstances.id, instanceId))
    ...
    if (existingRouting) throw conflict('Channel instance already has a routing configured')
  ...
  .get('/channel-routings', async (c) => {
    ...
    const rows = await db.select().from(channelRoutings).orderBy(desc(channelRoutings.createdAt))
  ...
  .patch('/channel-routings/:id', async (c) => {
    ...
    const [existing] = await db.select().from(channelRoutings).where(eq(channelRoutings.id, channelRoutingId))
  ...
  .get('/channels/status', async (c) => {
    ...
    const instances = await db.select().from(channelInstances).orderBy(channelInstances.type)
    ...
    const countMap = new Map(conversationCounts.map((r) => [r.channelInstanceId, Number(r.count)]))
