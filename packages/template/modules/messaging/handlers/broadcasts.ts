...
export const broadcastsHandlers = new Hono()
  ...
  .get('/', async (c) => {
    ...
    const [rows, [{ total }]] = await Promise.all([
      db.select().from(broadcasts).orderBy(desc(broadcasts.createdAt)).limit(limit).offset(offset),
  ...
  .get('/:id/recipients', async (c) => {
    ...
    const condition = status
      ? and(eq(broadcastRecipients.broadcastId, id), eq(broadcastRecipients.status, status))
  ...
  .post('/:id/recipients', async (c) => {
    ...
    const [existing] = await db.select({ id: broadcasts.id }).from(broadcasts).where(eq(broadcasts.id, id))
    ...
    const summary = await parseAndCreateRecipients(db, id, csvText, variableMapping, {
      saveAsLabel,
    })
  ...
  .post('/:id/send', async (c) => {
    ...
    await deps.scheduler.add('broadcast:execute', { broadcastId: id }, { singletonKey: id })
  ...
  .post('/:id/cancel', async (c) => {
    ...
    await db.update(broadcasts).set({ status: newStatus, updatedAt: new Date() }).where(eq(broadcasts.id, id))
  ...
  .post('/:id/retry-failed', async (c) => {
    ...
    const resetRows = await db
      ...
      .where(and(eq(broadcastRecipients.broadcastId, id), eq(broadcastRecipients.status, 'failed')))
  ...
  .post('/:id/audience-add', async (c) => {
    ...
    await db.update(broadcasts).set({ totalRecipients: total, updatedAt: new Date() }).where(eq(broadcasts.id, id))
