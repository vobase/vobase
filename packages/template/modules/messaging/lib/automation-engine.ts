...
function parseAudience(raw: unknown): AudienceFilter {
  ...
  return parsed.success
    ? parsed.data
    : audienceFilterSchema.parse({ excludeOptedOut: true });
}
...
async function resolveAudienceForRule(
  ...
  if (rule.audienceResolverName) {
    ...
    if (!resolver) {
      throw new Error(
        `[automation-engine] Unknown audience resolver: ${rule.audienceResolverName}`,
      );
    }
...
function renderVariables(
  ...
  const ctx: Record<string, unknown> = {
    ...
    phone: contact.phone ?? '',
    ...Object.fromEntries(
      Object.entries(attrs).map(([k, v]) => [`attributes.${k}`, v]),
    ),
  };
...
export async function evaluateRecurringRules(now: Date = new Date()): Promise<{
  ...
  const dueRules = await db
    ...
    .where(
      and(
        ...
        or(
          sql`${automationRules.nextFireAt} IS NULL`,
          lte(automationRules.nextFireAt, now),
        ),
      ),
    );
  ...
  const counts = await forEachRuleBounded(dueRules, async (rule) => {
    ...
    if (!firstStep) {
      logger.warn(
        '[automation-engine] Recurring rule has no step 1 — skipping',
        { ruleId: rule.id },
      );
    ...
    const { audience, resolverVariables } = await resolveAudienceForRule(
      db,
      rule,
    );
...
async function forEachRuleBounded<T, R>(
  items: T[],
  fn: (item: T) => Promise<R>,
): Promise<R[]> {
  const results: R[] = [];
...
async function fireRule(args: FireRuleArgs): Promise<number> {
  ...
  const audience =
    contactOverride ??
    (await resolveAudience(db, parseAudience(rule.audienceFilter)));
  ...
  const inserted = await db
    ...
    .onConflictDoNothing({
      target: [
        automationRecipients.ruleId,
        automationRecipients.contactId,
        automationRecipients.dateValue,
      ],
