...
import { createConversation, isAgentAssignee, reopenConversation } from './conversation'
import { insertMessage } from './messages'
import { findOrCreateContact } from './routing'
import { transition } from './state-machine'

export const MARKETING_STOP_KEYWORDS = ['stop', 'unsubscribe', 'opt out', 'cancel'] as const
 ...
 */
export async function handleInboundMessage(deps: InboundDeps, event: MessageReceivedEvent): Promise<void> {
  ...
  try {
    ...
    if (trimmed && (MARKETING_STOP_KEYWORDS as readonly string[]).includes(trimmed)) {
    ...
    if (existingConversation) {
      await appendAndRoute(deps, existingConversation, event, contact.id, channelInstanceId)
      return
    }

    // 3b. Check for reopenable conversation: resolved within idle window
    const channelInfo = await resolveChannelAdapter(db, channels, channelInstanceId)
    ...
    if (reopenable) {
      ...
      const reopenResult = await reopenConversation({ db, realtime }, reopenable.id, idleWindowMs)

      if (reopenResult.ok) {
        // Re-read the conversation after reopen to get updated state
        const [reopened] = await db.select().from(conversations).where(eq(conversations.id, reopenable.id))

        if (reopened) {
          await appendAndRoute(deps, reopened, event, contact.id, channelInstanceId)
          return
        }
      }
    ...
    const [channelRouting] = await db
      ...
      .where(and(eq(channelRoutings.channelInstanceId, channelInstanceId), eq(channelRoutings.enabled, true)))

    if (!channelRouting) {
      logger.warn('[conversations] No enabled channelRouting for channel instance', {
        channelInstanceId,
        from: event.from,
      })
      return
    ...
    await appendAndRoute(deps, conversation, event, contact.id, channelInstanceId)
  } catch (err) {
...
async function handleEchoMessage(
  ...
  const [existingResult, contactResult] = await Promise.all([
    event.messageId
      ? db.select({ id: messages.id }).from(messages).where(eq(messages.externalMessageId, event.messageId))
      : Promise.resolve([]),
    db.select({ id: contacts.id }).from(contacts).where(eq(contacts.phone, event.from)),
  ])
...
async function appendAndRoute(
  ...
  const contentType = mediaResult?.contentType ?? (MEDIA_TYPES.has(event.messageType) ? event.messageType : 'text')
  ...
  if (event.content || mediaResult || MEDIA_TYPES.has(event.messageType)) {
    ...
    const isMediaType = ['image', 'document', 'audio', 'video'].includes(contentType)
    if (isMediaType && (mediaResult?.media?.[0]?.storageKey || contentType === 'audio' || contentType === 'video')) {
      scheduler
        .add('messaging:process-media-caption', { messageId: message.id }, { retryLimit: 2, retryDelay: 5 })
        .catch((err) => {
  ...
  await upsertSessionIfWindowed(db, conversation.id, channelInstanceId, channelInfo)

  await routeByAssignee({ db, scheduler, channels, realtime }, conversation, event.content, contactId, channelInfo)
}
...
async function linkCampaignOrigin(
  ...
  } else if (winner === 'broadcast' && broadcastRow && !currentMeta.broadcastId) {
    await db
      .update(conversations)
      .set({
 ...
 */
async function resolveInstanceId(db: VobaseDb, candidateId: string): Promise<string | null> {
...
async function resolveChannelAdapter(
  ...
  return {
    type: instance.type,
    adapter: channels.getAdapter(channelInstanceId) ?? channels.getAdapter(instance.type),
  }
}
...
async function uploadInboundMedia(
  ...
  for (const media of event.media) {
    const filename = media.filename ?? `${event.messageId ?? Date.now()}.${extensionFromMime(media.mimeType)}`
