...
import { channelInstances, channelRoutings, channelSessions, contacts, conversations } from '../schema'
...
describe('upsertSession', () => {
  it('creates a new session with window_open state', async () => {
    ...
    const [session] = await db.select().from(channelSessions).where(eq(channelSessions.conversationId, CONV_ID))
  ...
  it('upserts existing session (refreshes window)', async () => {
    ...
    const [first] = await db.select().from(channelSessions).where(eq(channelSessions.conversationId, CONV_ID))
    ...
    const all = await db.select().from(channelSessions).where(eq(channelSessions.conversationId, CONV_ID))

    expect(all).toHaveLength(1)
    expect(all[0].windowExpiresAt.getTime()).toBeGreaterThanOrEqual(first.windowExpiresAt.getTime())
  })
})
