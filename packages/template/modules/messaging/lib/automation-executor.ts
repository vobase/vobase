...
export async function executeAutomationStep(
  ...
  if (execution.status !== 'running') {
    logger.info(
      '[automation-executor] Execution not in running status — skipping',
  ...
  const [step] = await db
    ...
    .where(
      and(
        ...
        eq(automationRuleSteps.sequence, stepSequence),
      ),
    );
  if (!step) {
  ...
  const [nextStep] = await db
    ...
    .where(
      and(
        eq(automationRuleSteps.ruleId, rule.id),
        eq(automationRuleSteps.sequence, stepSequence + 1),
      ),
    );
  ...
  const adapter =
    channels.getAdapter(rule.channelInstanceId) ??
  ...
  await executeSendBatch({
    ...
    async loadBatch(_offset, limit) {
      const rows = await db
        ...
        .where(
          and(
            eq(automationRecipients.executionId, executionId),
    ...
    async updateRecipient(recipient, result) {
      if (result.success) {
        const nextStepAt =
          nextStep && nextStep.delayHours != null
            ? new Date(Date.now() + nextStep.delayHours * 3_600_000)
            : null;
        await db
