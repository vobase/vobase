...
import { msgMemCells } from './schema';
      ...
      try {
        ...
        const lastCell = (
          await db
            ...
            .from(msgMemCells)
            .where(eq(msgMemCells.threadId, threadId))
            .orderBy(desc(msgMemCells.createdAt))
            .limit(1)
        )[0];
        ...
        const [cell] = await db
          ...
          .returning({ id: msgMemCells.id });

        // Queue formation job
        await scheduler.add('messaging:memory-formation', {
          cellId: cell.id,
        });
      } catch (err) {
