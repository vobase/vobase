...
import { embedChunks } from '../../../knowledge-base/lib/embeddings';
...
export async function processMemCell(
  ...
  const [claimed] = await db
    ...
    .where(and(eq(msgMemCells.id, cellId), eq(msgMemCells.status, 'pending')))
    .returning();
  ...
  const [otherProcessing] = await db
    ...
    .where(
      and(
        eq(msgMemCells.threadId, cell.threadId),
        eq(msgMemCells.status, 'processing'),
        ne(msgMemCells.id, cellId),
      ),
    )
    .limit(1);
  ...
  if (otherProcessing) {
    await db
      .update(msgMemCells)
      .set({ status: 'pending' })
      .where(eq(msgMemCells.id, cellId));
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      // Insert episode
      await tx.insert(msgMemEpisodes).values({
        cellId,
      ...
      if (eventLogs.length > 0) {
        await tx.insert(msgMemEventLogs).values(
...
async function loadCellMessages(
  db: VobaseDb,
  cell: typeof msgMemCells.$inferSelect,
