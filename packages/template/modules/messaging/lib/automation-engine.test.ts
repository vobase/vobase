...
import { computeNextFireAt, evaluateDateRelativeRules, evaluateRecurringRules } from './automation-engine'
...
async function seedRule(db: VobaseDb, overrides: Partial<typeof automationRules.$inferInsert> = {}): Promise<string> {
...
describe('evaluateRecurringRules', () => {
  ...
  it('fires rule when nextFireAt is due, enqueues execute-step, advances cursor', async () => {
    ...
    expect(recipients.map((r) => r.phone).sort()).toEqual(['+6591000001', '+6591000002'])

    const enqueued = schedulerJobs.filter((j) => j.name === 'automation:execute-step')
  ...
  it('skips inactive rules', async () => {
    ...
    const result = await evaluateRecurringRules(new Date('2026-04-17T02:00:00Z'))
...
describe('evaluateDateRelativeRules', () => {
  ...
  it('dateValue equals today-in-rule-tz, not UTC', async () => {
    ...
    expect(typeof dateValue === 'string' ? dateValue : (dateValue as unknown as Date).toISOString().slice(0, 10)).toBe(
      '2026-04-17',
    )
  })
  ...
  it('respects sendAtTime HH:MM match', async () => {
    ...
    const early = await evaluateDateRelativeRules(new Date('2026-04-17T00:00:00Z'))
    expect(early.recipientsInserted).toBe(0)

    // 2026-04-17 09:00 SGT = 01:00 UTC — matches
    const onTime = await evaluateDateRelativeRules(new Date('2026-04-17T01:00:00Z'))
