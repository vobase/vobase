...
import { automationRecipients, automationRuleSteps, automationRules, contacts } from '../schema'
...
export async function rescheduleDateRelativeRecipients(): Promise<{
  ...
  for (const rule of rules) {
    ...
    const stepSeqs = [...new Set(queued.map((r) => r.currentStep).filter((s): s is number => s != null))]
    const steps = await db
      ...
      .where(and(eq(automationRuleSteps.ruleId, rule.id), inArray(automationRuleSteps.sequence, stepSeqs)))
    const offsetByStep = new Map(steps.map((s) => [s.sequence, s.offsetDays ?? 0]))
    ...
    const attrByContact = new Map(
      contactRows.map((c) => [c.id, ((c.attributes ?? {}) as Record<string, unknown>)[dateAttribute]]),
    )
    ...
    for (const r of queued) {
      ...
      if (typeof actual !== 'string' || actual.slice(0, 10) !== expectedContactDate) {
        staleIds.push(r.id)
      }
    }
