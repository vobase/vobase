...
describe('handleInboundMessage — new conversation', () => {
  it('creates contact and conversation for unknown sender', async () => {
    ...
    // Contact was created
    const [contact] = await db.select().from(contacts).where(eq(contacts.phone, '+6599999999'))
  ...
  it('skips silently when no enabled channel routing exists', async () => {
    ...
    await db.update(channelRoutings).set({ enabled: false }).where(eq(channelRoutings.id, 'cr-wa'))
...
describe('handleInboundMessage — held mode', () => {
  ...
  it('accepts message silently (no auto-reply) for on-hold conversation', async () => {
    ...
    const allMessages = await db.select().from(messages).where(eq(messages.conversationId, 'conv-held'))
...
describe('handleInboundMessage — ai mode', () => {
  ...
  it('schedules agent-wake job with debounce for WhatsApp', async () => {
    ...
    expect((replyJob?.data as { conversationId: string }).conversationId).toBe('conv-ai')
    // cancelWake + singletonKey dedup, 1s delay for advisory lock release
    expect(replyJob?.opts).toBeDefined()
    expect((replyJob?.opts as { singletonKey: string }).singletonKey).toContain('agents:agent-wake:')
...
describe('handleInboundMessage — supervised mode', () => {
  ...
  it('schedules agent-wake job for supervised mode', async () => {
    ...
    const replyJob = schedulerJobs.find((j) => j.name === 'agents:agent-wake')
    expect(replyJob).toBeDefined()
    expect((replyJob?.data as { conversationId: string }).conversationId).toBe('conv-sup')
  })
})
...
describe('handleInboundMessage — human mode', () => {
  ...
  it('records inbound message but does not schedule agent-wake job', async () => {
    ...
    // Message stored
    const allMessages = await db.select().from(messages).where(eq(messages.conversationId, 'conv-human'))
    const inbound = allMessages.find((m) => m.messageType === 'incoming')
    expect(inbound).toBeDefined()
    ...
    // No delivery job
    const deliveryJob = schedulerJobs.find((j) => j.name === 'messaging:deliver-message')
...
describe('handleInboundMessage — media upload', () => {
  ...
  it('uploads media to storage and sets correct contentType', async () => {
    ...
    const allMessages = await db.select().from(messages).where(eq(messages.conversationId, 'conv-media'))
    const inbound = allMessages.find((m) => m.messageType === 'incoming')
    expect(inbound).toBeDefined()
    expect(inbound?.contentType).toBe('image')
    expect(inbound?.content).toBe('[image]')
    expect((inbound?.contentData as { media: unknown[] }).media).toHaveLength(1)
  })
...
describe('handleInboundMessage — session upsert', () => {
  ...
  it('does not create session for web channel without messaging window', async () => {
    ...
    const sessions = await db.select().from(channelSessions).where(eq(channelSessions.conversationId, 'conv-web'))
...
describe('handleInboundAction', () => {
  ...
  it('schedules agent-wake job and stores action as message', async () => {
    ...
    const replyJob = schedulerJobs.find((j) => j.name === 'agents:agent-wake')
    expect(replyJob).toBeDefined()
    expect((replyJob?.data as { conversationId: string }).conversationId).toBe('conv-action')

    // Verify the action was stored as an inbound message
    const [actionMsg] = await db.select().from(messages).where(eq(messages.conversationId, 'conv-action'))
...
describe('reply-link tiebreak — automation vs broadcast', () => {
  async function seedAutomationRecipient(opts: { contactId: string; phone: string; sentAt: Date; status?: string }) {
    const [rule] = await db
  ...
  async function seedBroadcastRecipient(opts: { contactId: string; phone: string; sentAt: Date; status?: string }) {
