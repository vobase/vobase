...
import { list as listConversations } from '../service/conversations'
...
export const summarizeInboxTool = defineAgentTool({
  ...
  lane: 'standalone',
  prompt:
    'First call when triaging — cheap, read-only, safe to run multiple times per wake. Filter by `tab` and `owner` to narrow scope before pulling individual conversations.',
