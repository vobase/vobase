  ...
  defineIndexContributor({
    ...
    render: (ctx) => {
      ...
      return [
        ...
        '- Staff directs you to message the customer (push an artifact, relay an answer, send an update) → do it now: `send_file` for a drive file or public URL (requires approval), `reply_contact` or `send_card` for an agent-authored message. All work on this wake.',
        '- Note is staff answering a prior question of yours and the customer is still waiting → relay the answer to the customer now with `reply_contact` (or `send_card`). Do not just leave an `add_note` and wait for the next inbound.',
        '- Note is pure internal coaching with no customer waiting → act on it via memory/proposals/workspace write; no customer message needed.',
        '',
        '**Step 3 — close the loop with the staff member** with `consult_staff` (address the note author). Short, conversational, ≤10 words; lead with the verb.',
