  ...
  defineIndexContributor({
    ...
    render: (ctx) => {
      ...
      return [
        ...
        "- Staff directs you to push a specific artifact (drive file or public URL) to the customer → `send_file` works on this wake (requires approval); the agent-authored tools `reply_contact` / `send_card` do not (they only fire on the customer's next inbound wake).",
        '- Note is staff answering a prior question of yours and only text is needed → leave a brief `add_note` confirming the answer; the answer goes to the customer on their next inbound wake (or, if urgent, ask staff to send it themselves).',
        '',
