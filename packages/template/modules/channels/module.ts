...
const channels: ModuleDef = {
  ...
  init(ctx) {
    ...
    // `resolveKindSpec` reads `config.kind: 'notification'` and routes the
    // dispatch through the `vobase-platform-notification` vault namespace.
    // Registering under a distinct channel name lets `findNotificationChannel`
