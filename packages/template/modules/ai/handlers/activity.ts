...
export const activityHandlers = new Hono().get('/activity', async (c) => {
  ...
  if (params.agentId)
    conditions.push(sql`${cd}->>'agentId' = ${params.agentId}`);
  if (params.category)
    conditions.push(sql`${cd}->>'eventType' LIKE ${`${params.category}.%`}`);
  if (params.type) conditions.push(sql`${cd}->>'eventType' = ${params.type}`);
  if (params.channelType)
    conditions.push(eq(messages.channelType, params.channelType));
  if (params.contactId)
    conditions.push(
  ...
  if (params.timeFrom)
    conditions.push(gte(messages.createdAt, new Date(params.timeFrom)));
  if (params.timeTo)
    conditions.push(lte(messages.createdAt, new Date(params.timeTo)));
