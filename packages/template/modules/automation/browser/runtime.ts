...
const SERVER_URL = typeof __VOBASE_SERVER_URL__ !== 'undefined' ? __VOBASE_SERVER_URL__ : window.location.origin
const PRODUCT_NAME = typeof __PRODUCT_NAME__ !== 'undefined' ? __PRODUCT_NAME__ : 'Vobase'
...
// ─── HTTP Helper ────────────────────────────────────────────────────
function apiRequest<T>(method: string, path: string, body?: unknown): Promise<{ status: number; data: T | null }> {
...
function showPairingUI(): void {
  ...
  title.style.cssText = 'margin:0 0 16px;font-size:18px;color:#111;padding-right:24px;'
  ...
  const doSubmit = async () => {
    ...
    try {
      const { status, data } = await apiRequest<{ apiKey: string }>('POST', '/pairing/redeem', {
        code,
...
async function pollForTasks(): Promise<void> {
  ...
  try {
    const { status, data } = await apiRequest<TaskPayload>('GET', '/script/poll')
    if (status === 401) {
