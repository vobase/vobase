...
import { CopyIcon, MonitorIcon, PowerOffIcon, RefreshCwIcon } from 'lucide-react'
...
function CountdownTimer({ expiresAt }: { expiresAt: string }) {
  ...
  return (
    <span className={`text-xs tabular-nums ${isExpired ? 'text-destructive' : 'text-muted-foreground'}`}>
  ...
  return (
    <div className="p-6 space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Connect a browser session to enable automation</p>
      </div>
      ...
      <Card className="gap-2">
        ...
        <CardContent className="space-y-4">
          ...
          {pairingResult && !generateMutation.isPending && (
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <span className="font-mono text-3xl font-bold tracking-[0.25em] select-all">{pairingResult.code}</span>
                <Button variant="ghost" size="sm" className="h-8 px-2 text-muted-foreground" onClick={copyCode}>
          ...
          {generateMutation.isError && (
            <p className="text-xs text-destructive">Failed to generate code. Please try again.</p>
          )}
          ...
          <div className="flex gap-2">
            <Button
              ...
              className="gap-1.5"
            >
              <RefreshCwIcon className={`size-3.5 ${generateMutation.isPending ? 'animate-spin' : ''}`} />
          ...
          <ol className="space-y-3 text-sm">
            <li className="flex gap-3">
              ...
              <span className="text-muted-foreground leading-5">
                Install the <span className="font-medium text-foreground">TamperMonkey</span> browser extension from the
            ...
            <li className="flex gap-3">
              ...
              <span className="text-muted-foreground leading-5">
                Click <span className="font-medium text-foreground">Install</span> when TamperMonkey prompts you to
            ...
            <li className="flex gap-3">
              ...
              <span className="text-muted-foreground leading-5">
                Open the target site (e.g. <span className="font-medium text-foreground">WhatsApp Web</span>, any
                automation target)
              </span>
            </li>
            <li className="flex gap-3">
              ...
              <span className="text-muted-foreground leading-5">
                Enter the pairing code above in the floating TamperMonkey panel on that page
              </span>
            </li>
          </ol>
        </CardContent>
        ...
        <CardContent>
          ...
          {!sessionsLoading && sessions.length === 0 && (
            <div className="flex flex-col items-center gap-2 py-6 text-center">
              ...
              <p className="text-sm text-muted-foreground">No active sessions. Pair a browser to get started.</p>
            </div>
          )}
                ...
                <div key={session.id} className="flex items-center justify-between text-sm">
                  <div className="min-w-0">
                    <p className="font-medium truncate">
                      {parseBrowserName(session.browserInfo?.userAgent as string | undefined) ?? session.id}
                    </p>
                    {session.pairedAt && (
