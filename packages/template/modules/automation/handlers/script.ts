...
import { cancelTask, claimNextTask, completeTask, failTask } from '../lib/tasks'
...
export const scriptHandlers = new Hono()
  ...
  .post('/report', async (c) => {
    ...
    const [task] = await db
      ...
      .where(and(eq(automationTasks.id, taskId), eq(automationTasks.sessionId, sessionId)))
      .limit(1)
    if (!task) {
    ...
    if (task.status !== 'executing') {
      return c.json({ error: `Task is in '${task.status}' state, expected 'executing'` }, 409)
    }
