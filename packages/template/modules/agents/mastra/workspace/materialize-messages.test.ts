...
describe('materializeMessages', () => {
  it('formats a basic text conversation', async () => {
    ...
    const result = await materializeMessages(mockDb(msgs) as never, 'conv_abc123')

    expect(result).toContain('# Conversation conv_abc123')
    expect(result).toContain('Channel: whatsapp | Status: active | Assignee: agent:booking')
  ...
  it('formats image without caption', async () => {
    ...
    const result = await materializeMessages(mockDb(msgs) as never, 'conv_img2')
  ...
  it('formats audio with and without caption', async () => {
    ...
    const result = await materializeMessages(mockDb(msgs) as never, 'conv_audio')
  ...
  it('formats sticker', async () => {
    ...
    const result = await materializeMessages(mockDb(msgs) as never, 'conv_sticker')
  ...
  it('prefixes staff messages with [Staff]', async () => {
    ...
    const result = await materializeMessages(mockDb(msgs) as never, 'conv_staff')
  ...
  it('shows footer with limit note when at capacity', async () => {
    ...
    expect(result).toContain("(5 messages shown. Use 'vobase recall <query>' for older history.)")
  })

  it('handles missing conversation meta gracefully', async () => {
    const msgs = [makeMessage()]

    const result = await materializeMessages(mockDb(msgs, null) as never, 'conv_no_meta')

    expect(result).toContain('Channel: unknown | Status: unknown | Assignee: unassigned')
  })
  ...
  it('formats interactive content type same as text', async () => {
    ...
    const result = await materializeMessages(mockDb(msgs) as never, 'conv_interactive')
