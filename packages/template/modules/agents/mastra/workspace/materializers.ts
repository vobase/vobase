...
import { channelInstances, contacts, conversations } from '@modules/messaging/schema'
 ...
 */
export async function materializeState(db: VobaseDb, conversationId: string): Promise<string> {
  const rows = await db
    .select({
 ...
 */
export async function materializeProfile(db: VobaseDb, contactId: string): Promise<string> {
  const rows = await db
    .select({
      name: contacts.name,
 ...
 */
export async function materializeBookings(_db: VobaseDb, _contactId: string): Promise<string> {
 ...
 */
export async function materializeRelevant(_db: VobaseDb, _conversationId: string): Promise<string> {
  return ''
}
