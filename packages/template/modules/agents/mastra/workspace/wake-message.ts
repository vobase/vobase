...
const HISTORY_HINT =
  'Full conversation history is at conversation/messages.md if you need more context.';
...
export function buildWakeMessage(options: WakeMessageOptions): string {
  ...
  switch (trigger) {
    case 'scheduled_followup': {
      const reason =
        (payload?.reason as string | undefined) ?? 'Check in with contact';
      return `Scheduled follow-up: ${reason}. Read conversation/messages.md for context.`;
    }
    case 'supervisor': {
      const instruction =
        (payload?.instruction as string | undefined) ?? 'Review conversation';
      return `Supervisor instruction: ${instruction}. Read conversation/messages.md for context.`;
    }
    case 'manual': {
      const reason =
        (payload?.reason as string | undefined) ?? 'Agent wake requested';
...
function buildInboundWakeMessage(
  messages: WakeMessageEntry[],
  budget: number,
): string {
  ...
  if (omitted > 0) {
    parts.push(
      `(${omitted} earlier message(s) not shown — read conversation/messages.md for full history)`,
    );
  }
