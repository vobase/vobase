...
describe('command input validation (no DB)', () => {
  ...
  test('reassign fails with invalid target spec', async () => {
    const result = await conversationCommands.reassign(
      ['badspec'],
      {},
      fakeCtx,
    );
  ...
  test('mention fails with only target, no note', async () => {
    const result = await conversationCommands.mention(
      ['role:ops'],
      {},
      fakeCtx,
    );
  ...
  test('remind fails without --channel flag', async () => {
    const result = await conversationCommands.remind(
      ['contact-1', 'Hello'],
      {},
      fakeCtx,
    );
  ...
  test('follow-up fails with invalid delay', async () => {
    const result = await conversationCommands['follow-up'](
      ['abc'],
      {},
      fakeCtx,
    );
