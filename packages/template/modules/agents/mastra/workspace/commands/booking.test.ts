...
describe('booking commands', () => {
  describe('check-slots', () => {
    ...
    it('includes service name in header when provided', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain(
        'Available slots for 2026-04-15 (haircut):',
      );
    });
  ...
  describe('book', () => {
    ...
    it('creates a booking with service and datetime', async () => {
      ...
      expect(result.stdout).toMatch(
        /^Booked: BK-.{8} — haircut at 2026-04-15T10:00:00Z$/,
      );
    });
    ...
    it('includes notes when provided', async () => {
      const result = await cmd(
        ['2026-04-15T10:00:00Z'],
        { service: 'massage', notes: 'Prefer deep tissue' },
        stubCtx,
      );
      expect(result.exitCode).toBe(0);
  ...
  describe('reschedule', () => {
    ...
    it('reschedules a booking to a new datetime', async () => {
      const result = await cmd(
        ['BK-ABC12345', '2026-04-16T14:00:00Z'],
        noFlags,
        stubCtx,
      );
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toBe(
        'Rescheduled: BK-ABC12345 → 2026-04-16T14:00:00Z',
      );
    });
  ...
  describe('cancel', () => {
    ...
    it('includes reason when provided', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toBe(
        'Cancelled: BK-ABC12345 (Customer changed mind)',
      );
    });
