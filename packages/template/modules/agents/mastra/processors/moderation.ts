...
const MODERATION_NOTICE = '[Content moderated: message blocked by content policy]'
...
function buildModeratedContent(originalContent: unknown): unknown {
  if (originalContent && typeof originalContent === 'object' && 'format' in originalContent) {
    return {
 ...
 */
export function createModerationProcessor(config?: ModerationConfig, onBlock?: OnBlockCallback): InputProcessor {
  ...
  return {
    ...
    async processInput(args: ProcessInputArgs): Promise<ProcessInputResult> {
      ...
      if (content.length > maxLength) {
        ...
        abort(`Content blocked: exceeds max length (${content.length} > ${maxLength})`, {
          retry: true,
