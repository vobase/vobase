...
export const dashboardHandlers = new Hono().get('/dashboard', async (c) => {
  ...
  const [attentionResult, activeResult, resolvedResult, avgResponseResult] = await Promise.all([
    ...
    db
      .select({ count: count() })
      .from(conversations)
      .where(and(eq(conversations.status, 'resolved'), gte(conversations.resolvedAt, todayStart))),
