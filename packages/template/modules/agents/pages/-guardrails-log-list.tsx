...
async function fetchLogs(
  ...
  const res = await agentsClient.guardrails.logs.$get({
...
export function GuardrailsLogList() {
  ...
  if (isError) {
    return <p className="text-sm text-destructive text-center py-8">Failed to load moderation logs.</p>
  }
        ...
        <div key={log.id} className="flex items-start justify-between gap-3 rounded-lg border p-3">
          <div className="flex-1 min-w-0 space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              <Badge variant={log.reason === 'blocklist' ? 'destructive' : 'default'}>
            ...
            {log.blockedContent && <p className="text-xs text-muted-foreground truncate">{log.blockedContent}</p>}
          </div>
