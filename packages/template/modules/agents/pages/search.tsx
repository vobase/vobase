...
function highlightTerms(text: string, query: string): React.ReactNode {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (regex.test(part)) {
      result.push(
        <mark key={part} className="bg-transparent font-semibold text-foreground not-italic">
          {part}
        </mark>,
      )
    } else {
    ...
    <div className="p-6 max-w-3xl mx-auto">
                ...
                <CardContent>
                  <div className="flex items-center justify-between gap-3 mb-2">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      <Progress value={scorePct} className="h-1.5 w-16" aria-label={`${scorePct}% relevance`} />
      ...
      {!searchQuery && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">Enter a query above to search your knowledge base</p>
        </div>
      )}
    </div>
  )
