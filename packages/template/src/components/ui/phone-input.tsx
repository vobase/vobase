...
// shadcn-override-ok: show only the national portion in the field — the
// country popover already exposes the dial code, so the input renders
// `9123 4567` instead of `+65 9123 4567`. The store `value` stays full E.164.
function PhoneInputField(props: React.ComponentProps<"input">) {
  ...
  const countryCode = useStore((state) => state.country);
  ...
  const onChange = React.useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // If the user typed/pasted a full international number (with +), keep all
      // digits — the store's country-detection effect will switch the popover.
      // Otherwise treat input as the national portion and prefix the dial code.
      let nextDigits: string;
      if (startsWithPlus) {
        nextDigits = rawDigits;
      } else if (rawDigits.startsWith(dialDigits) && dialDigits.length > 0) {
        // User typed dial code at the start without a +; treat as full number.
        nextDigits = rawDigits;
      } else {
        nextDigits = dialDigits + rawDigits;
      }

      const newValue = nextDigits ? `+${nextDigits}` : startsWithPlus ? "+" : "";
      store.setState("startsWithPlus", startsWithPlus);
      store.setState("value", newValue);
    },
    [store, onChangeRef, isDisabled, isReadOnly, dialDigits],
  );

  const displayValue = React.useMemo(() => {
    if (!value) return "";
    const digits = value.startsWith("+") ? value.slice(1) : value;
    // Strip the leading dial code so the field shows only the national portion.
    const national =
      dialDigits && digits.startsWith(dialDigits)
        ? digits.slice(dialDigits.length)
        : digits;
    if (!national) return "";
    // Group every 4 digits for readability: "91234567" → "9123 4567".
