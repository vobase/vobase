import { AlertTriangleIcon, BotIcon, CheckIcon, ShieldIcon, TagIcon, WrenchIcon, ZapIcon } from 'lucide-react'
...
function formatAssignee(id: string | undefined, resolve?: ResolveParticipantName): string {
  if (!id) return 'unassigned'
  if (id.startsWith('agent:')) {
    const rest = id.slice(6)
    const name = resolve?.(id) ?? resolve?.(rest)
    return name ?? `${rest.charAt(0).toUpperCase()}${rest.slice(1).replace(/-/g, ' ')}`
  }
  return resolve?.(id) ?? id
}

/** Human-readable description for an activity event */
export function activityDescription(event: ActivityEvent, resolveName?: ResolveParticipantName): string {
  ...
  switch (type) {
    ...
    case 'handler.changed': {
      ...
      return to ? `Assigned to ${formatAssignee(to, resolveName)}` : 'Unassigned from staff'
    }
...
export function activityIcon(type: string) {
  if (type.startsWith('escalation')) return <AlertTriangleIcon className="size-3 text-red-500" />
  if (type === 'handler.changed') return <ZapIcon className="size-3 text-yellow-500" />
  if (type === 'session.completed' || type === 'conversation.resolved' || type === 'attention.reviewed')
  ...
  if (type === 'agent.draft_generated') return <WrenchIcon className="size-3 text-amber-500" />
  if (type.startsWith('guardrail')) return <ShieldIcon className="size-3 text-orange-500" />
  if (type === 'session.created' || type === 'conversation.created')
    return <BotIcon className="size-3 text-violet-500" />
  if (type === 'label.added' || type === 'label.removed') return <TagIcon className="size-3 text-violet-500" />
