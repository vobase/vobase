...
export function convertMemoryPart(part: { type: string; [key: string]: unknown }): NormalizedPart[] {
  if (part.type === 'text') {
...
export function getMessageParts(content: MemoryMessageContent): NormalizedPart[] {
  if (typeof content === 'string') {
    return content ? [{ type: 'text', text: content }] : []
  }
  if (Array.isArray(content)) {
    return content.flatMap((p) => convertMemoryPart(p as { type: string; [key: string]: unknown }))
  }
  if (content && typeof content === 'object' && 'parts' in content) {
    return (content as { parts: { type: string; [key: string]: unknown }[] }).parts.flatMap((p) => convertMemoryPart(p))
  }
  return []
}
...
function extractContentMetadata(content: MemoryMessageContent): Record<string, unknown> | undefined {
  if (content && typeof content === 'object' && !Array.isArray(content) && 'metadata' in content) {
