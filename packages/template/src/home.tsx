...
import { AlertTriangle, ArrowRight, CheckCircle, MessageSquare, Shield, Wrench } from 'lucide-react'
...
function eventIcon(type: string) {
  if (type.startsWith('session.completed') || type === 'attention.reviewed') return CheckCircle
  if (type.startsWith('escalation') || type === 'session.failed') return AlertTriangle
...
function eventDescription(
  ...
  const contact = (contactId && contactNames.get(contactId)) ?? contactId ?? 'a visitor'
  ...
  switch (type) {
    ...
    case 'escalation.created':
      return (
        <>
          <span className={n}>{agent}</span> <span className="font-medium text-destructive">escalated</span>{' '}
          <span className={n}>{contact}</span>
          {data.reason && (
            <span className={v}> — {data.reason.length > 60 ? `${data.reason.slice(0, 60)}…` : data.reason}</span>
          )}
        </>
      )
    ...
    case 'agent.tool_executed':
      return (
        <>
          ...
          <span className={n}>{data.toolName ? formatToolName(data.toolName) : 'a tool'}</span>
...
function HomePage() {
  ...
  const contactNames = new Map((contactsQuery.data ?? []).filter((c) => c.name).map((c) => [c.id, c.name as string]))
  ...
  return (
    <div className="flex flex-col gap-8 p-6 lg:p-10">
        ...
        {dashboardQuery.isPending ? (
          ...
          <>
            <div className="flex items-baseline gap-3">
              <span className="text-6xl font-black tracking-tight">{dashboard?.needsAttentionCount ?? 0}</span>
              <span className="text-lg text-muted-foreground">need your attention</span>
            </div>
            <div className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              <span>&middot;</span>
              <span>{formatResponseTime(dashboard?.avgResponseTimeMs ?? 0)} avg response</span>
            </div>
          </>
        )}
      </div>
            ...
            {activityQuery.isPending ? (
              ...
              <>
                {allEvents.map((event) => {
                  ...
                  const eventData = (event.contentData ?? {}) as ActivityEventData
                  const eventAgentId = (eventData.agentId as string | null) ?? null
                  const eventContactId = (eventData.contactId as string | null) ?? null
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1 truncate text-sm">
                        {eventDescription(eventType, eventAgentId, eventContactId, eventData, agentNames, contactNames)}
                      </span>
                      <span className="shrink-0 text-xs text-muted-foreground">
                        ...
                        <span className={eventContactId ? 'group-hover:hidden' : ''}>
                ...
                {activityQuery.hasNextPage && (
                  ...
                  >
                    {activityQuery.isFetchingNextPage ? 'Loading...' : 'Show more'}
                  </button>
                )}
              </>
            )}
      ...
      {(metricsQuery.isPending || agents.length > 0) && (
        <div>
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-muted-foreground">Agents</h2>
          {metricsQuery.isPending ? (
            ...
            <div className="flex gap-4 overflow-x-auto">
              {agents.map((agent) => (
                <div key={agent.agentId} className="flex shrink-0 items-center gap-3 rounded-lg px-3 py-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      <span className={successScoreColor(Math.round(agent.successScore * 100))}>
