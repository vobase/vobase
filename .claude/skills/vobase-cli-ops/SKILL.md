...
- **Every verb returns `Internal Server Error` (HTTP 500), not a clean `401`.** A *revoked or otherwise invalid* API key can make the tenant 500 on `/api/cli/verbs` (and `auth whoami`) instead of returning `401` — so it reads like a server outage rather than an auth problem. Disambiguate by curling the catalog with vs. without the key: `500` *with* the saved key but `401` *without* it ⇒ the key is the problem, not the server. Fix is the same as a stale key: re-login (`vobase auth login --url=... --config <name>`).
